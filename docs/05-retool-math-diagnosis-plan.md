...
### 6.5 · 协议预算阶梯：P2 之前的离线探针（2026-09-17）

§6 P2 的论证前提是"轮数是有效预算"（6 轮 → 单轮 1024 够用）。**该前提已被
`03-...-checklist.md` v4 探针反向证伪**：4B 关 thinking 后推理全走 content 通道，第一轮
烧断就再写不出完整围栏 → 命中"无代码即终局" → `max_rounds` 是**虚假预算**，有效单轨迹
预算 = `round_gen_tokens`。所以"预算够不够"必须在围栏协议下用**单轮预算**直接测，
不能靠轮数推导；这也是 P2 里"原生 `<tool_call>` 让 1024/轮够用"这一押注的对照基线。

离线探针（`probe_difficulty`，协议与训练同源；64 题固定 seed=42 前缀 × k=8 = 512 条轨迹）：

| `round_gen_tokens` / `max_context_tokens` | correct | 无 boxed | 末段截断 | 条件精度 |
|---|---|---|---|---|
| 3072 / 8192（run2 / P1b 原配方） | 162/512 = 31.6% | 65.0% | 58.6% | 90.3% |
| 3451 / 8192（= 该 ctx 的上限） | 194/512 = 37.9% | 59.0% | 52.3% | 92.4% |
| 4096 / 12288 | 221/512 = 43.2% | 52.0% | 45.3% | 90.0% |
| **5120 / 12288** | **268/512 = 52.3%** | 42.8% | 35.9% | **91.5%** |
| 6144 / 14336 | **290/512 = 56.6%** | 36.7% | **29.1%** | 89.4% |

**6144 点定案（2026-09-17，命中预写判据第 2 行：截断 25~30%）→ 停止抬预算，转提示层。**

斜率证据（每 +1024 token 单轮预算）：correct 增益 9.1pp（4096→5120）→ **4.3pp**（5120→6144），
截断降幅 9.4pp → **6.8pp**——两条斜率同时腰斩，拐点确认。外推到 ≤25% 需 ~6760/轮
（ctx 16384，T² 项 +30%、logits 项 +14%），只值 +2.6pp correct，**不划算**（判据第一行的
"再抬预算"分支就此关闭，不再试 6550/16384）。

**提示层（C）是下一根杠杆，接线已落地**（`--system_prompt_file`，train + probe 两侧）：
候选提示 `rlab/prompts/retool_math_concise.txt`——相对 preset 只增三处（不重述题面不重复推导 /
代码块短、无注释、只 print 最终值 / 拿到答案立即收尾给 boxed，不再补验证代码）。提示指纹进
签名（`-sp<hash6>`，默认档签名逐字不变），**因此 C 与 P1b 可以并行**：P1b 不传该 flag 即原配方。

```bash
CUDA_VISIBLE_DEVICES=0 python -m rlab.probe_difficulty --model_path /root/Qwen3.5-4B \
  --k 8 --max_questions 64 --dump_samples 20 \
  --max_rounds 2 --round_gen_tokens 6144 --max_context_tokens 14336 \
  --system_prompt_file rlab/prompts/retool_math_concise.txt \
  --out rlab_out/difficulty_probe_4b_v5_r6144_concise.jsonl
```

**C 的预写判据（单变量：只换提示，预算/种子/题集/打分口径不动）**：

| 观察（对照 6144 baseline：correct 56.6% / 无 boxed 36.7% / 截断 29.1% / 条件精度 89.4%） | 判定 | 下一步 |
|---|---|---|
| 无 boxed ≤30%（吃掉那 ≤6.9pp 地板的大半）**或** correct ≥62%，**且**条件精度 ≥85% | 提示有效 | 提示定版 → **用新提示重探 6144 全量表**（表与提示绑定，旧表作废）→ 带表训练 |
| 无 boxed 降了但条件精度 <85% | 提示在鼓励草率收尾（"只学格式"的另一种形态） | 提示作废，回到默认提示 + 靠预算 |
| 两项都没动 | 提示层无杠杆 | 接受 6144/14336 定版，直接起全量 30h 表 |

> **注意"提示改了 → 表必须重探"**：难度表是「模型 × 提示 × 预算」的联合产物，所以
> **全量 30h 探针必须在提示定版之后才启动**（这是本次先做提示 pilot 而不是直接起全量表的唯一原因）。

**C 结果（2026-09-17，concise 提示 @6144/14336）→ 协议初版定版**

| 臂（同 64 题 / k=8） | correct | 无 boxed | 截断 | 条件精度 | 墙钟 |
|---|---|---|---|---|---|
| 默认提示 | 56.6% | 36.7% | 29.1% | 89.4% | 852s |
| **concise 提示** | **312/512 = 60.9%** | **29.9%** | **22.7%** | 86.9% | **717s（−16%）** |

判据第 1 行三条全中（截断 22.7 ≤25 ✓、correct 60.9 ≥58 ✓、条件精度 86.9 ≥85 ✓）→
**定版三件套 = `--round_gen_tokens 6144 --max_context_tokens 14336
--system_prompt_file rlab/prompts/retool_math_concise.txt`**，签名应含 `-r2x6144` 与 `-sp8e0184`。

**如实记录的边界（防事后美化）**：

- 题级配对（同 64 题）：改善 22 题 / 退步 17 题 / 持平 25 题，平均 **+0.34 条/题**
  → **符号检验 p≈0.26，正确率增益未确证**；
- 站得住的是另外两条：截断 **−6.4pp**（轨迹级 ≈2.3σ）、**墙钟 −16%**（与指标无关的独立旁证）；
- 提示干的活与预期不同：**"收完尾不给框"的地板没动**（`无 boxed − 截断` 7.6→7.2pp），
  它省的是 token（等效白拿约 10% 预算），截断与正确率随之变好；
- 条件精度 89.4→86.9%（仍在 ≥85% 线内）——这是"省 token 换正确"的代价方向，
  后续数据若继续下探即说明提示在鼓励草率收尾；
- **定版的主要依据是截断率与算力，不是正确率增益**（后者记为"方向一致、未确证"）。

**机制已闭合（2026-09-17 题级配对检验，同 64 题 / k=8）**：

| 量 | 改善/退步题数 | 配对均值 | 符号检验 p |
|---|---|---|---|
| 正确条数/题 | 22 / 17 | **+0.344 条/题**（= +4.3pp） | 0.261（**未确证**） |
| 截断率/题 | 24 / 8 | **−6.4pp** | **0.004（确证）** |
| 平均生成长度 | 48 / 14（越短越好） | **−821 token（4755→3934，−17.3%）** | **<0.001（确证）** |

即因果链 **提示 → 长度 −17.3% → 截断 −6.4pp → 正确率 +4.3pp（弱）** 成立，且作用点
**在长度上**，不在"收尾格式"上（地板 7.6→7.2pp 未动）。量级换算：按预算阶梯的斜率
（+1024 token ⇔ 截断 −6.8pp），提示用 −821 token 换到 −6.4pp ≈ **免费获得约 1000 token
的单轮预算（+16%），而 T 一点没涨**（T²/logits 项不动）。这是提示层真正的价值所在。

**另一项更正（撤回 docs/03 的"TIR 对 4B 不必要"旁证）**：6144 下 `avg_code_ok = 0.484`
...
**合并后必查**：行数 ≈ 16,767、唯一题面 ≈ 16,756（缺 = 有片没跑完/崩溃）；
训练启动日志的 `[rollout] 难度过滤 … 表中缺失 m=0`。两片各自的 `--out` 都支持断点续跑。

> **【2026-09-17 首次分片的两个教训】** ①首次分片跑到 ~60% 时才发现："先按 `done`
> 过滤、再打乱切片"的顺序在**续跑**时会整体错位——本片跑去探另一片的题（重叠浪费，
> 且"互斥/并集 = 原集合"的不变量在续跑路径上失效；首跑完全看不出来）。已提为纯函数
> `probe_plan`（先打乱 → `--max_questions` → 切片 → 最后才按 `done` 过滤）+ 反证测试，
> **续跑前必须 `git pull`**。②**合并后必须核对覆盖率**：缺题不是报错，但
> `filter_qas_by_difficulty` 会把"表里没有的题"一律当丢弃 → 训练池被静默缩小
> （10,139/16,756 = 60% 时，可学题只剩 6,827）。

**提示不 promote 进 preset**（保持 file + flag 形态）：preset 一改，P1b 就不是 run2 原配方了。
等 P1b 跑完，再作为一次显式的"协议版本升级"合并进 preset。**全量 30h 表必须用 concise
提示探**（表 = 模型 × 提示 × 预算，旧表作废）。

**档（BI/非 BI）**：难度表是「模型 × 提示 × 预算」的联合产物，**不是 kernel 的产物**——
换 attention backend 只改数值实现的采样实现，不改变 p 的无偏估计（历史 BI 59.4% vs
非 BI 55.2% 是跨 run 对比，含协议漂移，不作证据）。默认按 pilot 口径（**非 BI**）探以省时间；
若要零争议，先做 16 题 A/B（同 k=8，一条带 `--vllm_batch_invariant --vllm_attention_backend
FLASH_ATTN`）量 p 的差与 BI 的降速，再决定 30 小时投在哪一档。

三条已定案的读法：

1. **条件精度四格恒定 90~92%**——多给的预算整块换成"答对了"，没有"学会猜"的迹象；
   瓶颈是"答不完"不是数学能力（与 samples dump 里 round1 烧穿占 80% 一致）。
2. **`无 boxed ≈ 截断 + ≤6.9pp`**：截断轨迹几乎必无 boxed，残余 ≤6.9pp 是"收完尾不给框"，
   只有提示层压得动。**故"截断 ≤35% 且 无 boxed ≤20%"联合不可达，判据只能用前者。**
3. **长度项不能关**：P1（ts=0）已反向证伪（§7.3.1，`m200 − p1s200 = +6.4pp`，p=0.002）。
   "截断比答错更差 → 学会缩短"这个陷阱的唯一杠杆是**压低截断率**，不是拆掉长度项。

**预写判据（6144 点跑完照此判，防事后解释）**：

| 观察（对照 5120/12288） | 判定 | 下一步 |
|---|---|---|
| 截断 ≤25% 且 correct ≥58% 且 条件精度 ≥85% | 协议定版 = 6144/14336 | 定档 → 全量 30h 探针 → 带表训练 |
| 截断 25~30% | 阶梯已近拐点 | 不再抬预算（T 的代价不划算），转提示层 |
| 截断 >30% | 阶梯失效 | 停预算线，转提示层 + 重估 P2 必要性 |

**成本账（别只盯探针）**：探针是单卡 vLLM 推理（一轮 ~15 分钟），训练端 T 是真金白银——
`logits` 项线性 T、SDPA math 回退的 T² 项随 T²（docs/04 §2）。5120/6144 **都没在训练端跑过**
（run2/bg1 都是 ctx 8192），故预算定版后第一件事是首 backward 的显存实测，
回退位 = `--micro_rows 1`（已定版）+ `ATTN_IMPL=flash_attention_2`（T² 项消失，docs/04 §6）。

**奖励侧要不要动？（2026-09-17 定案：不动，且方向相反）**

retool_math 的奖励里**没有任何格式项**：`total_reward_math` 只给
`acc = reward_correct_boxed(...)`，fmt 仅作监控记录（`total_reward_retool_math` docstring、
`code_w=0`）。所以"模型只学会了给 `\boxed{}`"在这个协议下**不可能从奖励里挣到分**；
加格式分/部分分/代码分才真的会制造这个病（阶段2 retool 家族四轮代码灭绝全在这条线上）。

真正的现象要换个名字：被强化的是"**在预算内尽快收尾**"。`trunc_shaping=-0.5` 让
"被截断"(-1.5) 比"答错"(-1) 更差，最快的改运路径就是"少写、早给框"——bg1 的实测签名
正是**收尾率 32%→50% 与正确率 30%→8.3% 同时发生**。这不是"学格式"，是"学会逃避截断罚分"。

而且它调奖励治不了：截断的那 ~36% 里有相当一部分在**本轮预算内根本不存在正确轨迹**，
奖励怎么设计都变不出可达的正确解，梯度只能教"少写"。这是**可行性问题不是激励问题**
（与三类代码灭绝同一条原则）。P1（ts=0）已把"拆掉长度项"证伪（+6.4pp 反过来）。
**故奖励配方一字不动**；若预算把截断压到 ≤25% 后仍出现表面收尾，再按单变量试
"把截断样本从 loss 里 mask 掉"（DAPO overlong filtering 的第三种口径，既非 -1.5 也非 -1）
——先列在这里，不预先实施。

**当场探测器（已落地，`rlab/health.py` 签名⑥ `surface`）**：窗口内**收尾率较开局涨 ≥10pp
且正确率掉 ≥5pp**（两条同时成立才判，且开局 fmt 率 ≥20% 才启用，避免 GSM8K 式"格式从 0
学起"的瞬态误报），告警文本带条件精度。bg1 那种崩溃不必再等 200 步后的 eval。
验证：合成 bg1 形态（收尾 57%→80%、正确 30%→10%）必报，两个对照组（只涨收尾 / 只掉正确）
均静默。

**"让超长和答错同分"怎么改、以及为什么现在不该改（2026-09-17 补）**

现状四档（prose 路径；`overlong_shaping=True` 对**单轮**轨迹是死开关——clen ≤ per
而 trigger = 2×per−256）：

| 轨迹形态 | acc | trunc_shaping | overlong | reward |
|---|---|---|---|---|
| 正常收尾 + 答对 | +1 | 0 | 0 | **+1.0** |
| 正常收尾 + 答错 | −1 | 0 | 0 | **−1.0** |
| 撞单轮上限（末段被切，未给出正确答案） | −1 | −0.5 | 0 | **−1.5** |
| 两轮用满且末尾被切 | −1 | −0.5 | −1.0 | **−2.5** |

要"同分"只有三条路：① `--trunc_shaping 0.0`（被判为 −1，**即 P1，已实测更差**）；
② `--trunc_shaping 0.25`（半档，未测）；③ 把截断样本从组基线与 loss 里 **mask** 掉
（advantage 恒 0——不是"同分"而是"没有分"，未测，预登记）。

**为什么 ① 不行——问题不在"罚多少"，在"罚谁"**：group_mean 下与"答错"的 advantage
差是固定的，`答对 +2.0 / 被截断 −0.5`——"少写猜一个"这条捷径的收益只有"真做对"的
**1/4**，模型仍然走它，因为**它一步可达，而"在预算内做对"对相当一部分截断样本物理上
不可达**。把 −0.5 抹平只删掉一条小捷径，不给不可达的那条路增加任何可达性；代价却是
实测的：P1 去掉罚分后 trunc **+14pp**、中段崩盘、收尾成绩 = BASE——outcome-only ±1 下
长度就是正确性的代理（组内更长者答对率 75.4%），没有反向项就长度膨胀 → 截断更多。

**定案**：`trunc_shaping` 是**二阶变量**（罚多少），**截断率是一阶变量**（罚到几条）。
截断 ≥45% 时罚分项变成主导梯度（bg1：收尾率涨、正确率崩）；截断 ≤25% 时它只是防止
长度膨胀的保护项。**先把截断率压到 ≤25%，在此之前不动奖励**；若届时仍出 surface
签名，按单变量先试 ③（mask），而不是回到 ①。

> 预算阶梯与 P1b **不冲突**：P1b 是 run2 原配方（r2x3072）的重现，本节的结论只作用于
> 其后的协议版本；但**提示层改动会污染 P1b**（提示是探针/训练/评测单一来源），
> 所以 P1b 与提示层改动必须排期分开。

### 6.6 · OOD 评测集（P0 剩余项，路线待定）
...
| P0-4 | OOD 评测集（AIME25） | `[ ]` | 路线待定，见 §6.6 |
