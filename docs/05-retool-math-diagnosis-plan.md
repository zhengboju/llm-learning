# retool_math（Qwen3.5-4B）run2 诊断与优化方案

> 状态：**诊断已定案；P1 消融已反向证伪（§7.3.1），下一步 = P1b（run2 原配方重跑，§6 P1b）**
> （2026-09-14 更新）
> 依据：run2 训练日志 `rlab/rlab_out/train_log2.txt`、得分记录 `rlab/rlab_out/record2.jsonl`、
> run2 评测 `eval_vllm_all.json`，以及参考项目
> [agentic-rl-lab/05-retool](https://github.com/KMnO4-zx/agentic-rl-lab/tree/main/05-retool)。
> 配套：迁移与协议层 checklist 见 `03-qwen35-4b-retool-math-checklist.md`；显存见 `04-qwen35-4b-gpu-memory-oom.md`。
> **进度看板见 §7**（`[x]` 已落地 / `[ ]` 待执行）。
> **§7.5.5（2026-09-18）**：p5 加量 + gen_update_steps=8 终局——step100 **+4.6pp 显著**
> （p=0.033）但 step300 回吐 −3.4pp（同款模式第三次）；剂量/快照错位已排除，
> 机制指向代码分支退化（code_ok 56%→10%），判别 H1/H2 用 `--code-layer` 分层分析。

---

## 0. 结论摘要

1. **step200 有小的真实增益，随后被训练抹平。**（2026-09-13 配对检验改判，§7.1）
   m200 = BASE **+5.8pp，McNemar p=0.006**（b=67/c=38，过 Bonferroni×3 校正）；
   m600 = BASE +0.6pp（p=0.796，噪声内）。**训练后期的退化被独立评测确证**，不是记录曲线的
   取样假象 —— 且退化模式与主嫌疑吻合：code 率 m200 62.6% → m600 **13.0%**，增益随工具使用一起消失。
2. **但这个"没学到"不是必然。** 参考项目在**同一基座、同一份 DAPO 数据、同样纯 outcome ±1**
   下把工具调用率从 1.0 推到 2.0 并稳定，Average@12 拿到 **+23.89pp（OOD）**。
3. **主嫌疑 `trunc_shaping=0.5` —— P1 消融已证伪（方向相反）**（§4.1、§7.3.1）：
   去掉它后 step200 增益消失（p1s200 45.2 = BASE；m200 − p1s200 = **+6.4pp,
   跨json配对 p=0.002**）且中段崩盘一次（p1s150 −7.4pp, p=0.001）。这个 0.5 惩罚
   是增益的**组成部分**，不是问题原因。残余问题（run2 晚期增益消失）的真因
   转向 §4.3（步数/lr）与 §4.2（协议）。
4. **次嫌疑是协议结构**：`max_rounds=2` 只允许 **1 次**代码执行，而参考实测平均 **2.98 轮**——
   参考报告"学到"的那个行为，在我们的协议里物理上不存在（§4.2）。
5. **测量侧缺口已补齐且已生效**（§9）：per-item 落盘、N-aware CI + McNemar、审计字段、run 偏离签名。
   2026-09-13 用 pod 新代码重评（§7.1）：m200 从"噪声内"**改判为显著**（+5.8pp, p=0.006）；
   baseB 同路径对照把 run-to-run 噪声实测为 **≈±1pp / 9 题翻转**（§9.6），
   此前 BASE 46.6↔45.6↔45.8 的漂移全部落在这个地板内，不再是未解之谜。

---

## 1. run2 训练侧事实（健康，但方向在退）

| 项 | 值 |
|---|---|
| 算法 / 基座 | `retool_math` / `/root/Qwen3.5-4B-text`（抽取的纯文本） |
| 配置 | `all_steps=1000`、`save_steps=200`、`num_pre_Q=8`、`gen_questions_per_attempt=4`、`lr=5e-6` |
| 长度/奖励 | `trunc_shaping=0.5`、`overlong_shaping=True`、`round_gen_tokens=3072`、`max_rounds=2`、`code_w=0.0`、`reward_switch_step=1e9` |
| 预算 | `max_prompt_length=1024`、`max_context_tokens=8192` |
| 数据 | `dapo_math` + `difficulty_probe_4b_v4.jsonl`、band `(0,1)` |
| 时间 | 15:14:11 → 21:26:17（6.20h），**719/1000 步**，~30 s/it，ETA ~1h55m |
| 偏离签名 | `retool_math-ts0.5-ol1-r2x3072-s1000x200-lr5e-06-d0-1` |

### 1.1 工程与数值：无异常

- 无 error / OOM / NaN；44 次权重推送**全部** `loaded 330/426`（`sync.py:18` 明确：stacked 融合
  如 q/k/v→qkv_proj 使 `loaded < sent` 是常态，计数恒定 = 没丢权重）。
- 采样：累计尝试 1516 / 有效上传 1264 组，丢弃 **14%**（零方差 212，**超长 0** → 预算自洽，
  `overlong_shaping` 的 trigger 真可达）；题目拉黑仅 69/20792。
- Loss 中位数 **~0.002**，step 240 后区间 [-0.011, +0.014]；两次孤立尖峰（0.95@step4、**8.29@step225**），
  之后再未复现。
- clip_frac 0.0165(320-490) → **0.0282 峰**(500-610) → 0.0095(620-710)，max 0.0808@step30；
  KL 1.5e-3、approx_kl 2.8e-3、`mean_ratio ∈ [0.9975, 1.0011]`、frac|d|>0.1 ≈ 5%；staleness 4–20 micro-step。
- **没有发散、没有梯度爆炸、没有信号恒死**（`fmt_const`/`acc_const`/`length_runaway` 均未触发）。
- lr 5e-6 下单步几乎不动，但 1000 步攒出了真实行为改变 —— 像是"每步太弱 + 总量太长"的组合（§4.3）。

### 1.2 质量曲线：acc 在 group≈386 见顶后单调回落

按 200 组窗口（每组 8 样本，≈1600 样本/行）汇总：

| groups | gen_ver | acc 率 | fmt 率 | code 率 | code_ok 率 | trunc 率 | avg_clen |
|---|---:|---:|---:|---:|---:|---:|---:|
| 0–200 | 0 | 38.5% | 51.6% | 56.9% | 45.1% | 33.8% | 2891 |
| 200–400 | 96 | **54.8%** | 65.1% | 62.5% | 51.0% | 31.9% | 2875 |
| 400–600 | 208 | 50.9% | 70.0% | 65.4% | 52.5% | 30.1% | 2613 |
| 600–800 | 320 | 49.4% | 75.3% | **66.6%** | **51.1%** | 24.8% | 2626 |
| 800–1000 | 432 | 42.5% | 71.3% | 59.6% | 38.8% | 28.2% | 2814 |
| 1000–1200 | 544 | 38.1% | 71.1% | 48.8% | 25.4% | 28.8% | 2874 |
| 1200–1263 | 672 | **38.1%** | **81.7%** | 53.2% | **23.2%** | **18.8%** | 2805 |

health 口径（32 组窗）：开局 **55.5%** → 峰值 **64.1%**（group 386 结束，gen_ver≈192）→ 末段 **42.2%**。
*（注：开局区间波动大，前 200 组的池化均值 38.5% 与首 32 组的 55.5% 都是真实取样差异，不是口径冲突。）*

**两个反向信号同时出现**：`fmt` 率一路涨到 81.7%（格式越写越规范），`code_ok` 率从峰值 52.5% 崩到
**23.2%**（代码越来越不能跑）。即"结构写对了，但答案错了、代码也跑不通了"。

### 1.3 健康检查告警（重放确认）

`health.py` 的 `maybe_check` 用 `self.fired` 去重，**同一告警只打印一次**——所以日志后期安静
**不等于**已恢复。我重放整个 record 得到：

| 触发位置 | 告警 | 内容 |
|---|---|---|
| group 32 | `retool_trunc` | 最近 32 组 >20% 末段被轮长上限切断，建议调大 `round_gen_tokens`/`max_rounds` |
| group 256 | `flat` | 256 组后 acc 净提升 <1pp（0.11→0.12）→ **没有学习签名** |
| group 288 | `decline` | acc 较开局下滑 >5pp（0.11→−0.08）→ **退化签名，建议早停** |

**`decline` 至今仍然 active**（末 32 组 acc ±1 口径 −0.156 vs 开局 +0.109）。

---

## 2. run2 评测侧事实

协议：`dapo_math dev`（held-out 500，**与训练同分布**）、N=500、seed=42、**greedy**、`--algo retool_math`、
`--base_path /root/Qwen3.5-4B`、`n_dropped_long=0`。checkpoint 已经过 `materialize_mm_ckpt` 物化。

| 模型 | acc | fmt | code_rate | code_ok_rate |
|---|---:|---:|---:|---:|
| BASE | 46.6% | 53.0% | 42.2% | 38.0% |
| **m200** | **51.6%** | 60.2% | **62.6%** | **53.0%** |
| m600 | 46.4% | 67.8% | **13.0%** | **9.6%** |

### 2.1 判定（§9 修正口径之后）

| 对比 | 差值 | 95% CI | p | 判定 |
|---|---|---|---|---|
| m200 − BASE (acc) | +5.0pp | ±6.2pp（[-1.2, +11.2]） | ≈0.11 | **噪声内** |
| m600 − BASE (acc) | −0.2pp | ±6.2pp | 0.95 | 噪声内 |
| m200 − m600 (acc) | +5.2pp | ±6.2pp | ≈0.10 | 噪声内 |
| m600 − BASE (fmt) | **+14.8pp** | ±6.0pp | <0.001 | **显著** |
| m600 − BASE (code_rate) | **−29.2pp** | ±5.2pp | <0.001 | **显著** |

**唯一能确证的：格式达标率真的涨了，工具调用率真的灭绝了。** 准确率的 +5.0pp 方向可信、幅度不可信。

### 2.2 两个口径事实（写进结论前必须知道）

- **`both ≡ acc` 是结构性恒等**：`acc=1` 的前提是 `_extract_last_boxed` 取到 boxed，而 `fmt=1`
  正是"取到 boxed"。所以"双达标"列不提供额外信息，`fmt` 在 boxed 口径下只测"有没有答案框"、不测对错。
- **同题配对检验本可提功效，但 run2 的数据永久补不回来**：三模型评的是同一批 500 题
  （同 seed/split），McNemar 功效远高于未配对两比例；而旧版 `eval_vllm_one.py` 只落盘聚合计数。
  §9 已把它改成默认落盘 per-item。

---

## 3. 诊断：三条独立证据指向同一结论

| # | 证据 | 来源 | 说明 |
|---|---|---|---|
| 1 | `decline` 签名从 group 288 起持续到训练结束 | 训练 record（health 重放） | 训练期可观测，独立于评测 |
| 2 | m600 ≈ BASE（−0.2pp） | held-out 评测 | 1000 步的**净收益 ≈ 0** |
| 3 | `code_rate` 62.6% → 13.0%（z≈−10.9） | held-out 评测 | 行为反转，不是噪声 |

外加**外部参照**：参考项目在同等条件下 OOD +23.89pp、工具使用不灭（§5）。所以这不是"4B 学不动"，
而是**我们的配方在某处把它推反了**。

---

## 4. 根因排序

### 4.1 主嫌疑：`trunc_shaping=0.5`（方向装反）

**结构事实（代码级）**：`rlab/reward.py:270` 明确 retool_math 是 outcome-only ——
"工具使用完全靠结果涌现，**不额外奖励 code_ok**"；preset `code_w=0.0`，
`total_reward_retool_math` 的签名里**根本没有 code / phase 项**。所以 reward 对"要不要写代码"
**零信号**，工具使用只受间接力支配。

**我们唯一的奖励偏离**就是这个 shaping，而它是**唯一与轮数正相关的惩罚**：写代码要占掉
`max_rounds=2` 里的一轮、显著抬高撞 `round_gen_tokens` 的概率 → 扣 0.5；纯 prose 是单发路径、
撞上限风险低。**唯一作用在工具使用上的力是负的。**

**参考项目的处置恰好相反**（`05-retool/readme.md`）：官方 released 代码里有
`score = min(0, score + (num_turns - 2) / 2 * 0.1)`（用在答错分支）—— 这是**保工具**的 shaping
（错但多调工具 → 只扣 0.8），而参考判断它与论文 outcome-only 说法冲突，**选择跟随论文、不加任何 shaping**。
两个方向的证据合起来指向同一结论：**我们加错了项。**

> 注意：这是**强假设而非已证事实**。相关性（截断率↓ + 长度↓ + 代码↓）方向未识别，
> 需要 P1 消融判决。参考只证明了"无 shaping 下工具不灭"，没有直接证明"我们的 shaping 灭了工具"。
>
> **【2026-09-13 P1 定案：本节假设已证伪，且方向相反】** 去掉惩罚的 P1 在同位置
> （step200）**丢掉了 run2 的全部增益**：p1s200 45.2 = BASE（b=61/c=61, p=1.0），
> 而 m200 51.6；跨 json 配对 **m200 − p1s200 = +6.4pp, p=0.002**。且 P1 中段崩盘
> 一次（p1s150 = 37.8，显著低于 BASE −7.4pp, p=0.001）。0.5 的截断惩罚是
> step200 增益的**组成部分**（压制"自由变长撞上限"），不是崩塌原因。
> 残余问题"run2 晚期增益为何消失"与 shaping 无关（P1 没跑到 400–600 段也未见
> 任何增益苗头），真因看 §4.2/§4.3。

### 4.2 次嫌疑：协议只允许 1 次代码执行

| | 参考 | run2 |
|---|---|---|
| 轮次 | `max_assistant_turns=6` / `max_code_calls=4` | **`max_rounds=2`（≤1 次执行）** |
| 实测 | 平均 **2.98 轮**、~2 次调用 | 至多 1 次 |
| 单轮预算 | 1024 | 3072 |
| 工具协议 | **原生 `<tool_call>`**（chat template `tools` 声明） | python 代码围栏检测 + 文本回填 |
| 系统提示 | 原生工具说明 | MAY 口径（"when … helps"） |

参考报告"学到"的行为是**调用 → 看结果 → 再调用 → 作答**的迭代式工具使用——**在我们的协议里不存在**。

当初砍轮数的依据是 `03-...-checklist.md` 的"4B 无代码即终局、`max_rounds` 是虚假预算、TIR 对 4B
可能不必要"。**这条前提被参考项目直接证伪**：Qwen3.5-4B base 的 smoke test 工具调用率
**87.5%（14/16）**、valid 85.4%、eval 下 1.69 calls/轨迹；而 run2 的 BASE 只有 42.2% 轨迹用过代码。
最可能的根因是**协议不同**：原生 `<tool_call>` 下模型一决定调用就停，单轮 1024 够用；围栏方案要求
模型一路生成到写出围栏才停，单轮预算被迫抬到 3072 才能容纳"思考+围栏"。
**"4B 不需要 TIR"很可能是围栏协议的伪影，不是模型属性。**

### 4.3 第三嫌疑：训练预算（步数 / lr / 存盘）

- **步数**：参考 200 步收工且 Average@12 全程单调；run2 的 m200 是**第一个也是最好的** checkpoint，
  之后退化。**多跑的 800 步是负收益。**
- **存盘**：`save_steps=200` 把 0–200 变成盲区；参考每 **50** 步存盘。
- **优化器口径不可直接搬**：参考 LoRA r32/lr 4e-5，run2 全参/lr 5e-6。run2 的
  `kl≈1.5e-3、clip<3%、mean_ratio≈1.0000` 说明单步几乎不动、1000 步才攒出一次行为反转。
  **但在 P1/P2 定性前不要动 lr/LoRA**，否则又是复合变量。
  > **【2026-09-17 反转】** 上面这句"单步几乎不动"的判断在旧协议下成立、在新协议下
  > **不成立**——见 §7.4：同 lr 5e-6，p3 在 400 样本内条件精度 82→56%、clen −67%。
  > **为什么当时读不出来（方法论教训，防再犯）**：①判据是**间接**的（kl/clip/mean_ratio
  > 小 ⇒ 推断"学得慢"），而"学得快"的代价（条件精度掉、clen 掉）当时**没有观测列**——
  > acc/fmt/clen 曲线都在，唯独没有"条件精度"这个能把收尾和正确分开的列，`trunc率↓`
  > 在旧口径里甚至被读成**好消息**；②诊断顺序被纪律锁死在 P1（shaping）→P2（协议），
  > lr 显式排在 P3"定性后才动"——**消融顺序本身把 lr 排在了最后一个**，而 P1/P2 的
  > 每次改动都同时改了梯度地形，lr 的最优值依赖地形 ⇒ 在地形没定之前测 lr，测出来的
  > 结论必然随地形成立而作废（p3 实测即反例）；③单 run 训练方差 ±4pp 掩盖了
  > "同 lr 下 window 0-160 与 160-320 的断崖差"这类**窗内对比**信号——跨 run 对比
  > 有噪声，窗内对比才是低噪信号，当时没有这个视角。

### 4.4 已排除 / 未解释

- **排除**：预算不自洽（超长丢弃 0）、权重同步失败（330/426 恒定）、NaN/发散、数据未过滤、
  MM 物化缺失（评测前已做）。
- **未解释**：退化率。参考 `degenerate_group_rate ≈ 0`（20 步跑为 0，200 步"缓降"），
  run2 零方差丢弃 **14–16%（212/1516）**。同 group=8 定义下差这么多，且参考做的是**全量 17k 不过滤**、
  run2 做了难度带过滤（本意是降退化）——见 §10。

---

## 5. 与参考项目 05-retool 的对照

| 维度 | 参考 05-retool | run2 |
|---|---|---|
| 基座 | `Qwen/Qwen3.5-4B` 原生多模态 | `/root/Qwen3.5-4B-text`（抽取纯文本） |
| 训练 | **LoRA r32，lr 4e-5** | **全参，lr 5e-6** |
| 步数 / 存盘 | **200** / 每 **50** | **1000** / 每 **200** |
| 组大小 | 8 | 8 |
| 奖励 | 纯 outcome ±1，**明确不加任何 shaping** | outcome ±1 **+ `trunc_shaping=0.5` + `overlong_shaping`** |
| 轮次 | 6 turns / 4 calls | **2 rounds（≤1 call）** |
| 单轮预算 | **1024** | **3072** |
| 工具协议 | 原生 `<tool_call>` | python 代码围栏 |
| 训练数据 | DAPO-Math-17k 全量 | 同数据 + 难度带过滤 |
| 评测 | **AIME25（OOD）30 题×12 采样**，temp1.0/top_p0.7 | **dapo_math dev（同分布）500 题**，greedy |
| 结果 | Average@12 **23.61% → 47.50%（+23.89pp）**，Format 25.83→76.11，code_calls 1.69→1.98（step90 峰 2.6），turns 2.69→2.98 | acc 46.6 → 51.6（m200，不显著）→ 46.4（m600）；code_rate 42.2→62.6→**13.0** |
| 自我限定 | "30 题×12 采样、单训练单种子，不足以宣称稳定复现"；8k vs 论文 16k 是最大妥协 | N=500 更扎实，但**没有 OOD 集** |

**结论差异的实质**：
1. **工具使用走向相反**（参考单调强化并稳定 ↔ run2 先强化后灭绝）。
2. **评测强弱不可比**：参考是 OOD、run2 是同分布。同分布只涨 5pp 且不显著，**比参考弱了不止一个量级**
   —— 所以"run2 没学到东西"这个判断跟参考一比只会更刺眼。
3. **"模型学会的顺序"没复现**：参考"先调规范 → 再写能跑的代码 → 最后写对解题有用的代码"；
   run2 第一阶段方向一致（code_ok 38→53）后整个链条反转（→9.6%）。

---

## 6. 优化方案

> **核心纪律：一次只改一个变量。** 当前的困境正是 4 个维度同时偏离的结果——再复合改动就学不到东西。

### P0 · 测量（已落地，§9）

`[x]` per-item 落盘 ｜ `[x]` N-aware CI + McNemar ｜ `[x]` `model_path`/`_meta` 审计 ｜
`[ ]` OOD 评测集（AIME25，见 §6.6）

### P1 · 单变量消融 `trunc_shaping`（**如果只做一件事，就做这个**）

```bash
bash rlab/run_gsm8k.sh retool_math /root/Qwen3.5-4B-text \
  --seed 42 --steps 300 --save_steps 50 --lr 5e-6 --trunc_shaping 0.0
# 签名 = retool_math-ts0-ol1-r2x3072-s300x50-lr5e-06-d0-1
```

> **`--lr 5e-6` 必须显式传**（2026-09-14 补）：`retool_math` preset **不含 `lr` 键**，
> 落到 BASE 默认 **1e-6**（实测 `get_config('retool_math')['lr'] == 1e-06`）——
> 漏传不报错、只会静默用 1e-6 跑完全程，签名写 `lr1e-06` 才看得出来。
> 而 run2 与 P1 的实测签名都是 `lr5e-06`（`rlab/train.py:455`：
> "preset 默认 1e-6；4B 加杠杆建议 5e-6"）。本文与 `04-...oom.md` 的命令块此前都漏了
> 这个 flag，照抄即静默偏离原配方。

**除 `trunc_shaping` 外一切与 run2 一致**（含 `max_rounds=2`），确保单变量。
步数 300 是因为参考 200 步收工、存盘 50 是为了不再出现 0–200 盲区。

**事前写死判据（防事后解释）**：

| 观察 | 判定 | 下一步 |
|---|---|---|
| 晚段 code_rate ≥40% 且 acc ≥ BASE | shaping 是主因 | 进 P2 |
| code_rate 仍 <20% | shaping 不是主因 | 跳过调参，直接进 P2（协议是主因） |
| code_rate 回升但 acc 不涨 | 工具不是瓶颈 | 转向数据/难度带（P4） |

> **2026-09-13 训练侧结果（§7.2）**：code_rate 66–69%（≥40% ✓）；acc 判定待 eval。
> 额外发现：去掉惩罚后 trunc **+14pp**、中段崩盘一次；且 300 步**没覆盖 run2 崩塌
> 发生段**（step 400–600），这一消融物理上测不到"能否防住晚期崩塌"。
>
> **2026-09-13 eval 定案（§7.3.1）**：判据第 3 行实际命中（code 高但 acc 不涨）且更糟 ——
> p1s200 = BASE、m200 − p1s200 = +6.4pp（p=0.002）。**本节假设反向证伪**，转向 P1b/P2。

成本：300 步 ≈ 2.6h（按 run2 实测 ~31 s/step）+ 评测。

### P1b · run2 原配方重跑（**当前唯一阻塞点，优先于 P2**）

目的（§7.3.1 定案 4）：① 定标 m200 的 +5.8pp 是**配方属性还是 run 随机性**
（跨 run 的 McNemar 只算了题级采样噪声，没算 RL 轨迹分岔）；② 拿回一个**活着的**
最佳 ckpt——m200 已灭失，OOD（AIME25）与 P2 对照都需要它。

```bash
bash rlab/run_gsm8k.sh retool_math /root/Qwen3.5-4B-text \
    --vllm_model_path /root/Qwen3.5-4B \
    --chat_template_kwargs '{"enable_thinking": false}' \
    --gen_gpu_mem 0.30 --micro_rows 1 --optim_8bit \
    --vllm_gen_logps --verify_gen_logps 5 \
    --difficulty_path rlab_out/difficulty_probe_4b_v4.jsonl \
    --out_dir rlab_out/retool_math_run2b \
    --seed 42 --steps 300 --save_steps 50 --lr 5e-6
# 签名 = retool_math-ts0.5-ol1-r2x3072-s300x50-lr5e-06-d0-1
```

**启动 30 秒内自检**：日志的 `signature=` 行必须含 **`lr5e-06`** 与 **`d0-1`**。
若见 `lr1e-06` → 漏传 `--lr`（静默 5 倍偏离）；若见 `nodiff` → 漏传 `--difficulty_path`。
两者都不报错，只有签名能暴露。（签名实测：传 `--lr 5e-6` = `...-lr5e-06-d0-1`，
不传 = `...-lr1e-06-d0-1`。）

与 run2 的唯一差异是**步数/存盘**（1000×200 → 300×50），奖励配方一字不改：
`trunc_shaping` **故意不传**（preset 内置 0.5 = 原配方），`--lr 5e-6` 必须显式传（见 P1 注解）。

启动注意三条：① `--out_dir` 必须给**新目录**——签名与 run2 不同（`s1000x200` → `s300x50`），
`guard_ckpt_collision` 不会放行同目录；② pod 上先 `git pull`；③ 若 `retool_math_run2b`
已有半截 `step_*`（09-13 首跑被旧版护栏误拦前可能已建目录），先确认是续跑还是换目录。

> **⚠️ 09-14 已知起跑故障：生成端被 OOM-killer 带走（无 traceback）**
>
> 09-14 首跑在起跑 ~1 分钟后中止，训练端报「生成端进程已退出」。**生成端没有任何
> Python traceback**——它是被信号杀死的：Qwen3.5 的 GDN 线性注意力层，vLLM 默认对
> prefill 走 **FlashInfer JIT 现场编译**（不是 AOT 预编译件，`/root/.cache/flashinfer/`
> 不跨 pod），而这段 nvcc 编译恰好落在 ref/gen/train 三方同时搬权重的启动期；本机 RAM
> 稳态已 ~50G/上限 60G，编译峰值直接顶爆。日志特征是 `ninja ... died with
> <Signals.SIGKILL>` **且零编译输出**（真编译错误会打一屏 nvcc 报错）+ 生成端无 traceback。
> **别顺着 train.py 那句"常见原因：显存不足/权重同步失败"去查 GPU。**
>
> 两个规避（**二选一，且都会改 prefill kernel，故签名已捕获**）：
> ① 推荐 `--vllm_gen_kwargs '{"gdn_prefill_backend": "triton"}'`（免 nvcc；代价只在
>    prefill，负载是 3072 token 的 decode 为主）。首跑自检：日志须有
>    `[rollout] 回读 gdn_prefill_backend = 'triton'`（回读 None = vLLM 静默忽略了该键名，
>    等于没修），且不再出现 `Using FlashInfer GDN prefill kernel`；签名多出
>    `-vkgdn_prefill_backend=triton` 后缀。
> ② 要保持 flashinfer 原配方：起跑前空载窗口预热编译一次
>    （`MAX_JOBS=1 python -c "import torch; torch.zeros(1,device='cuda');
>    from flashinfer.gdn_prefill import get_gdn_prefill_module as g; print(g())"`），
>    缓存落 `/root/.cache/flashinfer/`，之后训练启动只加载不编译。
>
> 确诊命令（尚未回填）：`cat /sys/fs/cgroup/memory.events`（看 `oom_kill`）+
> `cat /sys/fs/cgroup/memory.max`（**别信 `free -g`**，它报的是宿主内存）。

> **⚠️ 09-15 起跑故障：训练端第一个 backward 被 fla 护栏拦下**
>
> ```
> RuntimeError: Triton >= 3.4.0 and < 3.7.1 on Hopper GPUs produces incorrect
> results for gated chunk_bwd_dqkwg (see #640). Please upgrade Triton to
> >= 3.7.1 or install tilelang: `pip install tilelang`
> ```
>
> **这是护栏不是崩溃**：triton 坏区间 + Hopper 上 GDN 反向会算错梯度，fla 宁可
> raise。前向不受影响（loss 能算出来），死在 `engine.backward`。
> **修法 = 装 tilelang**（不动 triton 版本；该后端只接管这一个 kernel，前向与
> 生成端数值路径不变）。**绝不要**改掉那句 raise 或设 `FLA_DISABLE_BACKEND_DISPATCH=1`
> ——那是让坏 kernel 静默跑。
>
> 操作顺序（装包 → 验依赖 → 探针热身后再起跑）与三条判据见
> **`06-gdn-backend-tilelang.md`**；探针 `rlab/probe_gdn_backend.py`。
>
> **对 P1b 的两点影响**：① 本坑**不改变任何 cfg**，run 签名不变（`retool_math-ts0.5-…`
> 照旧），与 run2 的对照口径仍成立；② 但**必须先证实 run2 当时不在这个坑里**——
> 护栏 commit 是 09-11 才进 fla 的，而 run2（09-13）跑完了，说明当时护栏未生效，
> 究竟是"triton 当时不在坏区间"（run2 可信）还是"fla 当时无护栏"（run2 梯度被
> 静默污染），要用 `probe_gdn_backend.py --check-bad-triton` 量偏离后判定，
> 判定为后者则 §1.1「工程与数值：无异常」与后续全部结论都要加限定。

评测（对齐 §7.1 路线：`materialize_mm_ckpt` 物化 → `eval_vllm.py` 调度 → `eval_vllm_all.json`，
N=500 / greedy / seed=42），然后走 `pair_eval` 三方配对：

> **【2026-09-16 更新·真统一】不再需要手工物化**：训练存盘已直接产出 vLLM 可直读的多模态壳
> （`train.save_checkpoint` → `write_mm_checkpoint`，`step_N` 键名 `model.language_model.*` +
> 骨架复合 config/processor；`save_mm_checkpoint=True` 默认开）。python 直接指 `step_N` 即可。
> 09-16 之前的旧 `step_N`（文本格式）由 `eval_vllm_one.py` 自动物化到临时目录兜底
> （找不到骨架时给 `--mm_base /root/Qwen3.5-4B`）。下文的 `step_200_mm` 字样是当时的产物名。

```bash
cp eval_vllm_all.json eval_vllm_all.p1b.json     # 新一轮产物，别覆盖（旧 json 是唯一遗物）
python -m rlab.analysis --eval-json eval_vllm_all.p1b.json \
    --pair-json eval_vllm_all.run3-0913.json.bak --pair-a p1bs200 --pair-b m200
```

**事前写死判据**：`p1bs200 − BASE` 显著为正 **且** 与 m200 的差落在噪声内（|Δ| ≲1pp，
§9.6 地板）→ +5.8pp 是**配方属性**，§7.3.1 结论坐实、可推 P2；若 `p1bs200 ≈ BASE`
（差在噪声内）→ run 级随机性比 McNemar 算出来的大，§7.3.1 的跨 run 对比需整体降级。

### P2 · 协议与轮次（结构性，**必须与 P1 分开跑**）

`max_rounds=2` 在物理上只允许 1 次代码执行（`config.retool_math` 注释：round1 写代码 → round2 出 final）。
参考报告的行为需要多轮。预算由 `config.validate_retool_budget()` fail-fast 强制：

`rounds × per_round + max_prompt_length(1024) + 工具段预留(266) ≤ max_context_tokens(8192)`

| rounds | 单轮上限 |
|---:|---:|
| 2（run2） | 3451（用 3072） |
| 3 | 2300 |
| 4 | 1725 |
| **6（≈参考）** | **1150** |

**所以"6 轮"必然要求单轮降到 ~1024，这两件事耦合、不能只改一个。**
`03-...-checklist.md` 的"1024 截断 85%、必须 3072"是**围栏协议**下的结论；参考用原生 `<tool_call>`
时 1024/轮完全够用。**P2 的正解是换协议，而不是继续把单轮预算往上加**（后者只是治标，且会继续挤压轮数）。

```bash
# 目标签名 = retool_math-ts0-ol1-r6x1024-s300x50-lr5e-06-d0-1
```

备选（若协议改造太重）：`max_context_tokens 8192 → 16384`（论文设置）。参考自己说"预算翻倍是最值得
先试的改进"；但 2×H20 跑 4B 全参 + `04-...` 已有 OOM 记录，**先算显存再动**。

工程工作量：1–2 天（原生 tool 协议要动 chat template `tools` 声明 + 轨迹构造）。

### P3 · 训练预算

`[ ]` 步数上限 200–300（不再跑 1000）；`[ ]` `--save_steps 50`；`[ ]` 在 P1/P2 定性后再单独动 lr / LoRA。

### P4 · 数据侧

`[ ]` 无 `--difficulty_path` 的对照跑（回答"静态过滤是否真在降退化"，见 §10）；
`[ ]` 核对 pod 上 `rlab/datasets/dapo_math/train.jsonl` 的真实行数。

### 6.5 · 协议预算阶梯：P2 之前的离线探针（2026-09-17）

§6 P2 的论证前提是"轮数是有效预算"（6 轮 → 单轮 1024 够用）。**该前提已被
`03-...-checklist.md` v4 探针反向证伪**：4B 关 thinking 后推理全走 content 通道，第一轮
烧断就再写不出完整围栏 → 命中"无代码即终局" → `max_rounds` 是**虚假预算**，有效单轨迹
预算 = `round_gen_tokens`。所以"预算够不够"必须在围栏协议下用**单轮预算**直接测，
不能靠轮数推导；这也是 P2 里"原生 `<tool_call>` 让 1024/轮够用"这一押注的对照基线。

离线探针（`probe_difficulty`，协议与训练同源；64 题固定 seed=42 前缀 × k=8 = 512 条轨迹）：

| `round_gen_tokens` / `max_context_tokens` | correct | 无 boxed | 末段截断 | 条件精度 |
|---|---|---|---|---|
| 3072 / 8192（run2 / P1b 原配方） | 162/512 = 31.6% | 65.0% | 58.6% | 90.3% |
| 3451 / 8192（= 该 ctx 的上限） | 194/512 = 37.9% | 59.0% | 52.3% | 92.4% |
| 4096 / 12288 | 221/512 = 43.2% | 52.0% | 45.3% | 90.0% |
| **5120 / 12288** | **268/512 = 52.3%** | 42.8% | 35.9% | **91.5%** |
| 6144 / 14336 | **290/512 = 56.6%** | 36.7% | **29.1%** | 89.4% |

**6144 点定案（2026-09-17，命中预写判据第 2 行：截断 25~30%）→ 停止抬预算，转提示层。**

斜率证据（每 +1024 token 单轮预算）：correct 增益 9.1pp（4096→5120）→ **4.3pp**（5120→6144），
截断降幅 9.4pp → **6.8pp**——两条斜率同时腰斩，拐点确认。外推到 ≤25% 需 ~6760/轮
（ctx 16384，T² 项 +30%、logits 项 +14%），只值 +2.6pp correct，**不划算**（判据第一行的
"再抬预算"分支就此关闭，不再试 6550/16384）。

**提示层（C）是下一根杠杆，接线已落地**（`--system_prompt_file`，train + probe 两侧）：
候选提示 `rlab/prompts/retool_math_concise.txt`——相对 preset 只增三处（不重述题面不重复推导 /
代码块短、无注释、只 print 最终值 / 拿到答案立即收尾给 boxed，不再补验证代码）。提示指纹进
签名（`-sp<hash6>`，默认档签名逐字不变），**因此 C 与 P1b 可以并行**：P1b 不传该 flag 即原配方。

```bash
CUDA_VISIBLE_DEVICES=0 python -m rlab.probe_difficulty --model_path /root/Qwen3.5-4B \
  --k 8 --max_questions 64 --dump_samples 20 \
  --max_rounds 2 --round_gen_tokens 6144 --max_context_tokens 14336 \
  --system_prompt_file rlab/prompts/retool_math_concise.txt \
  --out rlab_out/difficulty_probe_4b_v5_r6144_concise.jsonl
```

**C 的预写判据（单变量：只换提示，预算/种子/题集/打分口径不动）**：

| 观察（对照 6144 baseline：correct 56.6% / 无 boxed 36.7% / 截断 29.1% / 条件精度 89.4%） | 判定 | 下一步 |
|---|---|---|
| 无 boxed ≤30%（吃掉那 ≤6.9pp 地板的大半）**或** correct ≥62%，**且**条件精度 ≥85% | 提示有效 | 提示定版 → **用新提示重探 6144 全量表**（表与提示绑定，旧表作废）→ 带表训练 |
| 无 boxed 降了但条件精度 <85% | 提示在鼓励草率收尾（"只学格式"的另一种形态） | 提示作废，回到默认提示 + 靠预算 |
| 两项都没动 | 提示层无杠杆 | 接受 6144/14336 定版，直接起全量 30h 表 |

> **注意"提示改了 → 表必须重探"**：难度表是「模型 × 提示 × 预算」的联合产物，所以
> **全量 30h 探针必须在提示定版之后才启动**（这是本次先做提示 pilot 而不是直接起全量表的唯一原因）。

**C 结果（2026-09-17，concise 提示 @6144/14336）→ 协议初版定版**

| 臂（同 64 题 / k=8） | correct | 无 boxed | 截断 | 条件精度 | 墙钟 |
|---|---|---|---|---|---|
| 默认提示 | 56.6% | 36.7% | 29.1% | 89.4% | 852s |
| **concise 提示** | **312/512 = 60.9%** | **29.9%** | **22.7%** | 86.9% | **717s（−16%）** |

判据第 1 行三条全中（截断 22.7 ≤25 ✓、correct 60.9 ≥58 ✓、条件精度 86.9 ≥85 ✓）→
**定版三件套 = `--round_gen_tokens 6144 --max_context_tokens 14336
--system_prompt_file rlab/prompts/retool_math_concise.txt`**，签名应含 `-r2x6144` 与 `-sp8e0184`。

**如实记录的边界（防事后美化）**：

- 题级配对（同 64 题）：改善 22 题 / 退步 17 题 / 持平 25 题，平均 **+0.34 条/题**
  → **符号检验 p≈0.26，正确率增益未确证**；
- 站得住的是另外两条：截断 **−6.4pp**（轨迹级 ≈2.3σ）、**墙钟 −16%**（与指标无关的独立旁证）；
- 提示干的活与预期不同：**"收完尾不给框"的地板没动**（`无 boxed − 截断` 7.6→7.2pp），
  它省的是 token（等效白拿约 10% 预算），截断与正确率随之变好；
- 条件精度 89.4→86.9%（仍在 ≥85% 线内）——这是"省 token 换正确"的代价方向，
  后续数据若继续下探即说明提示在鼓励草率收尾；
- **定版的主要依据是截断率与算力，不是正确率增益**（后者记为"方向一致、未确证"）。

**机制已闭合（2026-09-17 题级配对检验，同 64 题 / k=8）**：

| 量 | 改善/退步题数 | 配对均值 | 符号检验 p |
|---|---|---|---|
| 正确条数/题 | 22 / 17 | **+0.344 条/题**（= +4.3pp） | 0.261（**未确证**） |
| 截断率/题 | 24 / 8 | **−6.4pp** | **0.004（确证）** |
| 平均生成长度 | 48 / 14（越短越好） | **−821 token（4755→3934，−17.3%）** | **<0.001（确证）** |

即因果链 **提示 → 长度 −17.3% → 截断 −6.4pp → 正确率 +4.3pp（弱）** 成立，且作用点
**在长度上**，不在"收尾格式"上（地板 7.6→7.2pp 未动）。量级换算：按预算阶梯的斜率
（+1024 token ⇔ 截断 −6.8pp），提示用 −821 token 换到 −6.4pp ≈ **免费获得约 1000 token
的单轮预算（+16%），而 T 一点没涨**（T²/logits 项不动）。这是提示层真正的价值所在。

**另一项更正（撤回 docs/03 的"TIR 对 4B 不必要"旁证）**：6144 下 `avg_code_ok = 0.484`
（`max_rounds=2` 最多 1 次执行 → ≈48% 轨迹成功跑过代码），concise 下 0.441——3072 时代
观察到的 `code_ok≈0` 是**截断的产物**而非模型偏好，**TIR 路径在 4B 上是活的**。

**全量表的执行方式（多卡分片，2026-09-17）**：定版预算下实测 **~11 s/题** → 16,756 题
单卡 ≈**50h**（不是我早先估的 30h；预算抬高 + 多轮里沙箱/回卷期间引擎闲置是主因）。
`probe_difficulty` 新增 `--shard_index/--shard_count`（纯函数 `shard_items`，不变量
"各片互斥且并集 = 原集合"已锁进测试；切片在 `--max_questions` **之后**，要求各片共享
同一份打乱顺序）。两卡各一片、最后 `cat` 合并（行序无关，`load_difficulty_table` 按题面建 dict）：

```bash
CUDA_VISIBLE_DEVICES=0 python -m rlab.probe_difficulty --model_path /root/Qwen3.5-4B \
  --k 8 --max_rounds 2 --round_gen_tokens 6144 --max_context_tokens 14336 \
  --system_prompt_file rlab/prompts/retool_math_concise.txt \
  --shard_index 0 --shard_count 2 \
  --out rlab_out/difficulty_probe_4b_v5_r6144_concise_s0.jsonl
# GPU1：同一条命令改 --shard_index 1、--out ..._s1.jsonl
cat rlab_out/difficulty_probe_4b_v5_r6144_concise_s{0,1}.jsonl \
  > rlab_out/difficulty_probe_4b_v5_r6144_concise_full.jsonl
wc -l rlab_out/difficulty_probe_4b_v5_r6144_concise_full.jsonl   # 应 ≈ 全池题数
```

**合并后必查**：行数 ≈ 16,767、唯一题面 ≈ 16,756（缺 = 有片没跑完/崩溃）；
训练启动日志的 `[rollout] 难度过滤 … 表中缺失 m=0`。两片各自的 `--out` 都支持断点续跑。

> **【2026-09-17 首次分片的两个教训】** ①首次分片跑到 ~60% 时才发现："先按 `done`
> 过滤、再打乱切片"的顺序在**续跑**时会整体错位——本片跑去探另一片的题（重叠浪费，
> 且"互斥/并集 = 原集合"的不变量在续跑路径上失效；首跑完全看不出来）。已提为纯函数
> `probe_plan`（先打乱 → `--max_questions` → 切片 → 最后才按 `done` 过滤）+ 反证测试，
> **续跑前必须 `git pull`**。②**合并后必须核对覆盖率**：缺题不是报错，但
> `filter_qas_by_difficulty` 会把"表里没有的题"一律当丢弃 → 训练池被静默缩小
> （10,139/16,756 = 60% 时，可学题只剩 6,827）。

**提示不 promote 进 preset**（保持 file + flag 形态）：preset 一改，P1b 就不是 run2 原配方了。
等 P1b 跑完，再作为一次显式的"协议版本升级"合并进 preset。**全量 30h 表必须用 concise
提示探**（表 = 模型 × 提示 × 预算，旧表作废）。

**档（BI/非 BI）**：难度表是「模型 × 提示 × 预算」的联合产物，**不是 kernel 的产物**——
换 attention backend 只改数值实现的采样实现，不改变 p 的无偏估计（历史 BI 59.4% vs
非 BI 55.2% 是跨 run 对比，含协议漂移，不作证据）。默认按 pilot 口径（**非 BI**）探以省时间；
若要零争议，先做 16 题 A/B（同 k=8，一条带 `--vllm_batch_invariant --vllm_attention_backend
FLASH_ATTN`）量 p 的差与 BI 的降速，再决定 30 小时投在哪一档。

三条已定案的读法：

1. **条件精度四格恒定 90~92%**——多给的预算整块换成"答对了"，没有"学会猜"的迹象；
   瓶颈是"答不完"不是数学能力（与 samples dump 里 round1 烧穿占 80% 一致）。
2. **`无 boxed ≈ 截断 + ≤6.9pp`**：截断轨迹几乎必无 boxed，残余 ≤6.9pp 是"收完尾不给框"，
   只有提示层压得动。**故"截断 ≤35% 且 无 boxed ≤20%"联合不可达，判据只能用前者。**
3. **长度项不能关**：P1（ts=0）已反向证伪（§7.3.1，`m200 − p1s200 = +6.4pp`，p=0.002）。
   "截断比答错更差 → 学会缩短"这个陷阱的唯一杠杆是**压低截断率**，不是拆掉长度项。

**预写判据（6144 点跑完照此判，防事后解释）**：

| 观察（对照 5120/12288） | 判定 | 下一步 |
|---|---|---|
| 截断 ≤25% 且 correct ≥58% 且 条件精度 ≥85% | 协议定版 = 6144/14336 | 定档 → 全量 30h 探针 → 带表训练 |
| 截断 25~30% | 阶梯已近拐点 | 不再抬预算（T 的代价不划算），转提示层 |
| 截断 >30% | 阶梯失效 | 停预算线，转提示层 + 重估 P2 必要性 |

**成本账（别只盯探针）**：探针是单卡 vLLM 推理（一轮 ~15 分钟），训练端 T 是真金白银——
`logits` 项线性 T、SDPA math 回退的 T² 项随 T²（docs/04 §2）。5120/6144 **都没在训练端跑过**
（run2/bg1 都是 ctx 8192），故预算定版后第一件事是首 backward 的显存实测，
回退位 = `--micro_rows 1`（已定版）+ `ATTN_IMPL=flash_attention_2`（T² 项消失，docs/04 §6）。

**奖励侧要不要动？（2026-09-17 定案：不动，且方向相反）**

retool_math 的奖励里**没有任何格式项**：`total_reward_math` 只给
`acc = reward_correct_boxed(...)`，fmt 仅作监控记录（`total_reward_retool_math` docstring、
`code_w=0`）。所以"模型只学会了给 `\boxed{}`"在这个协议下**不可能从奖励里挣到分**；
加格式分/部分分/代码分才真的会制造这个病（阶段2 retool 家族四轮代码灭绝全在这条线上）。

真正的现象要换个名字：被强化的是"**在预算内尽快收尾**"。`trunc_shaping=-0.5` 让
"被截断"(-1.5) 比"答错"(-1) 更差，最快的改运路径就是"少写、早给框"——bg1 的实测签名
正是**收尾率 32%→50% 与正确率 30%→8.3% 同时发生**。这不是"学格式"，是"学会逃避截断罚分"。

而且它调奖励治不了：截断的那 ~36% 里有相当一部分在**本轮预算内根本不存在正确轨迹**，
奖励怎么设计都变不出可达的正确解，梯度只能教"少写"。这是**可行性问题不是激励问题**
（与三类代码灭绝同一条原则）。P1（ts=0）已把"拆掉长度项"证伪（+6.4pp 反过来）。
**故奖励配方一字不动**；若预算把截断压到 ≤25% 后仍出现表面收尾，再按单变量试
"把截断样本从 loss 里 mask 掉"（DAPO overlong filtering 的第三种口径，既非 -1.5 也非 -1）
——先列在这里，不预先实施。

**当场探测器（已落地，`rlab/health.py` 签名⑥ `surface`）**：窗口内**收尾率较开局涨 ≥10pp
且正确率掉 ≥5pp**（两条同时成立才判，且开局 fmt 率 ≥20% 才启用，避免 GSM8K 式"格式从 0
学起"的瞬态误报），告警文本带条件精度。bg1 那种崩溃不必再等 200 步后的 eval。
验证：合成 bg1 形态（收尾 57%→80%、正确 30%→10%）必报，两个对照组（只涨收尾 / 只掉正确）
均静默。

**"让超长和答错同分"怎么改、以及为什么现在不该改（2026-09-17 补）**

现状四档（prose 路径；`overlong_shaping=True` 对**单轮**轨迹是死开关——clen ≤ per
而 trigger = 2×per−256）：

| 轨迹形态 | acc | trunc_shaping | overlong | reward |
|---|---|---|---|---|
| 正常收尾 + 答对 | +1 | 0 | 0 | **+1.0** |
| 正常收尾 + 答错 | −1 | 0 | 0 | **−1.0** |
| 撞单轮上限（末段被切，未给出正确答案） | −1 | −0.5 | 0 | **−1.5** |
| 两轮用满且末尾被切 | −1 | −0.5 | −1.0 | **−2.5** |

要"同分"只有三条路：① `--trunc_shaping 0.0`（被判为 −1，**即 P1，已实测更差**）；
② `--trunc_shaping 0.25`（半档，未测）；③ 把截断样本从组基线与 loss 里 **mask** 掉
（advantage 恒 0——不是"同分"而是"没有分"，未测，预登记）。

**为什么 ① 不行——问题不在"罚多少"，在"罚谁"**：group_mean 下与"答错"的 advantage
差是固定的，`答对 +2.0 / 被截断 −0.5`——"少写猜一个"这条捷径的收益只有"真做对"的
**1/4**，模型仍然走它，因为**它一步可达，而"在预算内做对"对相当一部分截断样本物理上
不可达**。把 −0.5 抹平只删掉一条小捷径，不给不可达的那条路增加任何可达性；代价却是
实测的：P1 去掉罚分后 trunc **+14pp**、中段崩盘、收尾成绩 = BASE——outcome-only ±1 下
长度就是正确性的代理（组内更长者答对率 75.4%），没有反向项就长度膨胀 → 截断更多。

**定案**：`trunc_shaping` 是**二阶变量**（罚多少），**截断率是一阶变量**（罚到几条）。
截断 ≥45% 时罚分项变成主导梯度（bg1：收尾率涨、正确率崩）；截断 ≤25% 时它只是防止
长度膨胀的保护项。**先把截断率压到 ≤25%，在此之前不动奖励**；若届时仍出 surface
签名，按单变量先试 ③（mask），而不是回到 ①。

> 预算阶梯与 P1b **不冲突**：P1b 是 run2 原配方（r2x3072）的重现，本节的结论只作用于
> 其后的协议版本；但**提示层改动会污染 P1b**（提示是探针/训练/评测单一来源），
> 所以 P1b 与提示层改动必须排期分开。

### 6.6 · OOD 评测集（P0 剩余项，路线待定）

参考的结论建立在 AIME25（OOD）上；run2 只有同分布 dev。**没有 OOD 集就无法说"复现了 ReTool"。**

阻碍：pod 文档写明 **HF 网络不通**，而参考的 AIME25 走 `04-opsd/00-datasets.py --only aime25`，
本仓库没有对应的准备脚本。两条路线：

1. **modelscope 路线**（与 `data.py` 里 gsm8k 的既有做法一致）：新增 `--eval_task aime25` +
   `load_aime25_test()`（严格报错、不静默回落，照 `load_dapo_math_dev` 的模式）+ 下载脚本；
2. **手工放置**：把 AIME25 jsonl 放到 `rlab/datasets/aime25/test.jsonl`（格式 `{"Q":..., "A":...}`），
   只加加载器，最省事最可控。

评测口径对齐参考：30 题 × 12 采样、temp 1.0 / top_p 0.7、报 Average@12 / Pass@12 / Format。

---

## 7. 进度看板

> **`[x]` 的语义 = 代码已提交到 `origin/main`；在 pod 上生效必须先 `git pull`。**
> 【2026-09-12 教训】P0-1..3 在本地实现完但**没提交推送**，结果紧接着的一次重评
> 仍跑在旧代码上：`analysis.py` 打的是旧 6 列表格、json 里没有 `items`/`_meta` ——
> 又一次没留下 per-item，McNemar 还是算不了。"本地改完" ≠ "评测生效"。

| # | 项 | 状态 | 备注 |
|---|---|---|---|
| P0-1 | eval per-item 明细默认落盘（`--dump_items`） | `[x]` | `eval_vllm_one.py`，默认开；`--no-dump_items` 关闭 |
| P0-2 | 汇总判定改 N-aware CI + 同题配对 McNemar | `[x]` | `rlab/analysis.py`，旧固定 ±2pp 地板已降级 |
| P0-3 | 审计字段 `model_path` / `eval_protocol` / `_meta` | `[x]` | 修"事后无法核对评的是哪个 ckpt / BASE 用哪条路径" |
| P0-4 | OOD 评测集（AIME25） | `[ ]` | 路线待定，见 §6.6 |
| P0-5 | ckpt 撞名护栏（脚本硬拦 + train 签名扫描双层） | `[x]` | 2026-09-13 P1 的 step_200 覆掉 run2 的 step_200 后补，见 §7.2.1 |
| P0-6 | 健康检查滚动门（修检查点盲窗） | `[x]` | P1 实测组 112→240 约 1h 零检查，`fmt_low`/`length_runaway` 漏报 |
| P0-7 | `pair_eval` 跨 json 两两配对（对照已灭失模型） | `[x]` | m200 json 遗物 vs p1s200，见 §7.3 |
| 习惯 | run 偏离签名三处冗余 | `[x]` | `train.py`：wandb name / `run_info.json` / 启动日志 |
| P1 | `trunc_shaping 0.5 → 0.0`，300 步，存盘 50 | `[x]` | **已定案（§7.3.1）**：证伪且方向相反 —— 去掉惩罚丢掉 step200 全部增益（−6.4pp, p=0.002）、中段崩盘（p1s150 −7.4pp, p=0.001）。不延长 |
| P1b | **run2 原配方（ts0.5）重跑** 300 步 / 存盘 50 / 新 out_dir | `[ ]` | 定标 m200 增益是配方还是 run 随机性 + 拿回活的最佳 ckpt（m200 已灭失，OOD 需要）。**优先于 P2**。**命令 + 判据见 §6 P1b**（注意 `--lr 5e-6` 必须显式传）。**09-15 起跑前置**：装 tilelang 并跑 `rlab/probe_gdn_backend.py`（见 §6 ⚠️ 块 + `06-gdn-backend-tilelang.md`）；同时用 `--check-bad-triton` 判定 run2 是否跑在坏 triton 上 |
| — | GDN 反向护栏（triton 坏区间 × Hopper）| `[x]` | 09-15 非 P 序列故障。`rlab/probe_gdn_backend.py`（三条判据 + 两条反证 + JIT 预热）+ `rlab/tests/test_gdn_backend_cpu.py`（25 项 CPU 判据自检）+ `docs/06-gdn-backend-tilelang.md`。**pod 侧动作**：`pip install tilelang --no-deps` → 验依赖未被动 → 探针热身后起跑 |
| — | 抽取自检判据改口径（`max\|Δlogits\|>0.1` → 逐位相等主闸） | `[x]` | 2026-09-14，非 P 序列任务：0.1017 是**异路径对拍**在 bf16 下的正常累积，不是抽取有误。`rlab/extract_text_model.py` `_selfcheck` 重写为三层判据 + 反证控制；若重跑过抽取，本条是 P1b 的隐性前置 |
| P2 | 原生 `<tool_call>` + rounds 6 / per_round 1024 | `[ ]` | 与 P1 分开跑 |
| P3 | 步数 200–300 + 存盘 50 成为默认 | `[ ]` | |
| P4 | 无难度过滤对照 + `train.jsonl` 行数核对 | `[ ]` | 见 §10 |

### 7.1 P0 重评结果（2026-09-13，pod 新代码，dapo_math dev N=500 / greedy / seed=42）

| 模型 | acc | fmt | code | Δacc vs BASE | McNemar | 判定 |
|---|---:|---:|---:|---:|---|---|
| BASE | 45.8 | 51.8 | 42.2 | — | — | — |
| baseB（同路径重跑对照） | 44.8 | 50.8 | 42.2 | −1.0pp | p=0.180 (b=2/c=7, n=496) | 噪声内 → **run 噪声地板 ≈±1pp** |
| m200 | 51.6 | 60.2 | 62.6 | **+5.8pp** | **p=0.006 (b=67/c=38, n=496)** | **显著** |
| m600 | 46.4 | 67.8 | 13.0 | +0.6pp | p=0.796 (b=69/c=65, n=496) | 噪声内 |

三个要点：

1. **m200 翻案为显著正增益**（4 个模型对 BASE 共 3 重比较，Bonferroni 阈值 0.0167，p=0.006 过）。
   run2 的正确叙事不是"没学到"，而是"**step≈200 学到 +5.8pp，之后被训练抹平**"——与 §1.2 的
   acc 见顶（group≈386 / gen_ver≈192 ≈ step200）吻合。
2. **BASE 漂移之谜由 baseB 解掉**（§9.6）：同一权重、同一命令重跑，仅 9/500 题翻转、净 −1.0pp。
   之前三轮 BASE 46.6 / 45.6 / 45.8 的游走全部在此地板内；m200/m600 三轮逐位不变则说明
   它们的输出更"硬"（近并列 logits 少）。
3. ~~P1 的先验变强了~~（**已被 §7.2 修正**）：当时读到的"m600 增益消失与 code 崩塌（62.6→13.0）
   同向同步 → 正是 trunc_shaping 预测的模式"是 observational 吻合；P1 消融训练侧显示
   去掉惩罚无增益、trunc 反 +14pp —— 这个吻合更可能**相关而非因果**，真因更可能在
   协议/步数维度（§4.2/§4.3）。

遗留小项：`analysis.py` 目前只做 各模型 vs BASE；**m200 vs m600 的直接配对检验**（回答
"51.6→46.4 的回落本身是否显著"）需要一条两两比较的扩展，或 pod 上手动 `paired_counts`。

### 7.2 P1（ts0）训练侧结果（2026-09-13，300/300 步跑完，eval 待做）

Run 身份：签名 `retool_math-ts0-ol1-r2x3072-s300x50-lr5e-06-d0-1`（日志第 15 行核对无误），
除 `trunc_shaping` 0.5→0.0 外与 run2 同配置 —— **单变量纪律成立**。23:53→03:11 共 3h17m
（39.5 s/it，比 run2 的 ~31 s/it 慢 ~25%：轨迹更长，见下）。`save_steps=50` 首次拿到
0–300 全粒度 ckpt（step_50/100/150/200/250/300）。

工程健康：无 error/OOM/NaN；loss 峰 1.48（无 run2 式 8.29 尖峰）；18 次权重推送全
`loaded 330/426`；丢弃 11–14%（全部零方差，超长 0）；拉黑 24/20792；难度过滤行与 run2
逐字相同（1791200 → 20792）。

质量曲线（100 组窗，P1 vs run2 同组区间直接对齐）：

| 组 | gen_ver | P1 acc | run2 acc | P1 code | run2 code | P1 trunc | run2 trunc | P1 clen | run2 clen |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 0–100 | 48 | 37.4 | 38.9 | 61.1 | 53.0 | 28.5 | 27.0 | 2707 | 2394 |
| 100–200 | 96 | **23.6** | 38.1 | 63.2 | 60.9 | **48.4** | 40.6 | **3872** | 3387 |
| 200–300 | 144 | 41.1 | 51.2 | 63.4 | 60.2 | 46.5 | 33.1 | 3782 | 2958 |
| 300–400 | 208 | 48.2 | 58.4 | 66.2 | 64.8 | 39.8 | 30.6 | 3372 | 2793 |
| 400–500 | 256 | 47.8 | 54.2 | 66.0 | 62.9 | 43.5 | 30.0 | 3413 | 2501 |
| 500–553 | 288 | 47.6 | 46.7 | **69.3** | 66.0 | **45.5** | 31.6 | 3632 | 2861 |

训练侧四个结论（eval 前的中间判定）：

1. **去掉 trunc_shaping 没换来增益，反而在它管的地方变差。** 同组数位置（500–553）：
   P1 acc 47.6 ≈ run2 46.7、code 69.3 ≈ 66.0，但 trunc **45.5 vs 31.6（+14pp）**、clen +771。
   §4.1"惩罚方向装反"假设被削弱 —— 0.5 的惩罚确实在压制截断，去掉它没换来 acc/code。
2. **中段崩盘（组 100–224）**：acc 掉到 17–35%（100–200 窗 23.6%）、trunc 46–60%、
   clen ~3900，随后恢复到 ~48%。`decline` 告警（组 256，0.14→−0.27）如实捕获；
   run2 同位置无此崩盘 —— 无惩罚时模型先"自由变长撞上限"，再学会收着写。
3. **峰值与形状**：P1 滚动 32 组峰 **57.4%（组 524，贴着结束还在爬）**；run2 峰 64.1%
   （组 386，之后回落）。两条曲线形状不同（P1 崩-复-升 vs run2 升-峰-落），
   300 步对 P1 可能不够 —— 但先 eval 再决定是否延长。
4. **结构性限制**：run2 的 code 崩塌（eval m600=13%）发生在 step 400–600，
   **P1 只跑 300 步，物理上没测到"去掉惩罚能否防住晚期崩塌"** —— 这条消融的
   核心问题反而落在它的时间覆盖之外。

### 7.2.1 本轮暴露的两个工程缺陷（均已修复并测试）

1. **ckpt 撞名覆盖（实锤，第二次）**：P1 与 run2 共用 `out_dir=./rlab_out/retool_math`，
   P1 的 step_200 **已覆盖 run2 的 step_200 原始文本 ckpt** —— run2 的 m200（唯一显著
   正增益的模型）只剩 `step_200_mm` 合并副本身存。9/8 与 9/13 两次同类事故，脚本里的
   纯提示挡不住深夜启动的新 run。修复三层：`run_gsm8k.sh` 见旧 `step_*` 默认拒绝启动
   （`OUT_DIR_REUSE=1` 逃生口；`OUT_DIR` 环境变量可整体换目录，现在会透传给 train）；
   `train.py guard_ckpt_collision` 启动时签名级扫描（同签名重跑放行、**无签名按出处
   不明拒绝** —— run2 时代的 ckpt 没有签名字段）；存盘时二次核对。
   【2026-09-13 顺带修正】脚本级护栏首版只看脚本自己的 `OUT_DIR` 变量、不解析
   `"$@"` 里的 `--out_dir`，把 P1b 首跑（带了 `--out_dir rlab_out/retool_math_run2b`）
   误拦在共享目录上 —— 已修：护栏/record 归档均跟随用户 `--out_dir`（空格/等号
   两种写法都认，测试 +1）。
   **【后续，2026-09-13】`step_200_mm` 也被删除 → run2 的 m200 模型彻底灭失**
   （raw 已被 P1 覆盖、副本又删，RL 不可复现）。唯一遗物 = per-item 评测 json。
   已补 `analysis.py pair_eval`：跨两个 json 按题 qk 同题配对 McNemar ——
   **"活模型 p1s200 vs 已灭失的 m200"的单变量对照（同 step、唯一差异 shaping）
   仍可打完**，见 §7.3。教训升级：评测 json 与 ckpt 同级是承重产物，删之前先备份。
2. **健康检查盲窗**：`maybe_check` 旧门 `%16` + 每外层轮末才调一次 → 检查点稀疏，
   P1 实测组 112→240 约 1 小时零检查；期间 `fmt_low`（窗口 fmt 22%<25%）与
   `length_runaway`（clen 较开局 +43%）的触发条件**真的成立过但从未报告**（把
   重放对齐到真实检查点后与日志 3 条告警逐一对上，确认不是代码没到位）。
   修复：滚动门（距上次检查 ≥`check_every` 组即查）。测试 +6（60→66 项）。

### 7.4 P3 系（新协议 p3/p4）训练侧对照：lr 5e-6 vs 1e-6（2026-09-17）

新协议 = 定版三件套（6144/14336 + concise 提示 + 半表难度过滤）+ BI/FLASH_ATTN/triton +
`--vllm_logprobs_n 1`。p3（`retool_math_p3`，lr **5e-6**）跑了 400 样本后被杀（record 归档为
`.bak-0917-120701`）；p4（`retool_math_p4`，lr **1e-6**，同日重启）跑到 880 样本。两 run
窗口 0~160 的曲线**逐位相同**（61.3/74.4/82.4/65.0/55.0/21.9/4018）——同一份权重、同一
seed、同一表的前 20 组完全可复现，交叉验证了"记录端无噪声"。

| 窗口（样本） | lr | acc率 | fmt率 | **条件精度** | code率 | code_ok率 | trunc率 | avg_clen |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| 0~160 | — | 61.3% | 74.4% | 82.4% | 65.0% | 55.0% | 21.9% | 4018 |
| 160~320 | **5e-6** | **25.0%** | 45.0% | **55.6%** | 71.9% | 55.0% | **9.4%** | **2740** |
| 320~480 | **5e-6** | **5.0%** | 7.5% | 66.7% | 67.5% | **30.0%** | 8.8% | **1313** |
| 160~320 | **1e-6** | 54.4% | 63.7% | 85.3% | 63.1% | 54.4% | 31.9% | 4818 |
| 320~480 | **1e-6** | **67.5%** | 75.0% | **90.0%** | 76.2% | 58.8% | 15.0% | 3527 |
| 480~640 | 1e-6 | 62.5% | 72.5% | 86.2% | 66.9% | 50.6% | 21.9% | 5064 |
| 640~800 | 1e-6 | 69.4% | 81.2% | 85.4% | 62.5% | 49.4% | 15.6% | 3750 |
| 800~960 | 1e-6 | 55.0% | 63.7% | 86.3% | 60.0% | 43.8% | 35.0% | 5399 |

（p3 全会话：acc 35.5% / 条件精度 72.1%，且单调恶化中；p4 全会话：acc 62.3% / 条件精度 85.9%，
开局 61.3% → 高位震荡。）

**判读（p3 = run2 崩溃机制的加速重放，实锤 lr 是"步长"不是"方向"）**：

1. **p3 在 200 个样本内把 run2 用 1000 步走完的崩溃演完**：条件精度 82.4% → 55.6% →
   66.7%（窗内 66.7% 是小样本假象，acc 率 5% 是真的），avg_clen 4018 → 2740 → 1313
   （**-67%**）——"少写、早收尾"被 lr 5e-6 在几十个 optimizer step 内强化成主导策略。
   trunc 率同步降到 9%，即模型"成功"地不截断了，代价是全面放弃解题。**这就是
   `trunc_shaping` 与"预算内做对"这对矛盾在高速学习率下的解：模型选了前者。**
   → **结论：4B + 全参下 lr 5e-6 过大，"每步太弱"的判断（§4.3）在新协议下反转为
   "步子太大"。** 与 rfpp/阶段1 的经验一致：退化速度与 lr 正相关。
2. **p4（1e-6）是新协议下第一个健康的训练曲线**：acc 61.3% → 67.5% 峰值 → 55~69%
   高位震荡；**条件精度全程 85~90%**（无表面收尾迹象）；trunc 全程 9~35% 无单调趋势；
   avg_clen 在 3.5k~5.4k 无方向性漂移（`length_runaway` 未触发）。code_ok 55→44%
   缓降但 code 率稳定在 60~76%——**代码没有灭绝**（对比 run2 的 62.6→13.0%），且
   code_ok≈0.43 与探针口径一致。第 5 窗 acc 69.4% 与第 6 窗 55.0% 的波动属
   单 run 相邻窗正常幅度（阶段1 实测 2-4pp，本表窗口更大、波动更大）。
3. **仍有两个待验证项**：① acc 曲线是否只是"在 base 水平上震荡"——需 eval（尤其
   step_200/step_300 vs BASE 的配对），训练 acc 62~69% 的绝对水平**高于**探针 correct
   56~61%，方向是好迹象但不是证据（训练分布=带过滤的池+采样温度）；② 最后窗口
   trunc 35% / clen 5399 抬头——是波动还是长度膨胀的起点，继续观察
   （`length_runaway` 门限 1.35× 尚远）。

**决策（2026-09-17 定案）**：**lr = 1e-6（preset 默认）定版**，5e-6 消融反向证伪。
p4 继续跑到 300 步存满（save_steps=50），eval 走 §7.1 路线（物化 mm 壳 → N=500 greedy
dev），判据 = `p4s200 − BASE` 显著为正（McNemar）+ 条件精度 ≥85% 保持。此后若要再加
学习率，先跑 p4 的 eval 拿到基线再谈。

### 7.5 p5（retool_math_p5，同协议 200 步）eval + record 对账（2026-09-17）

**身份**：`retool_math-ts0-ol1-r2x6144-s200x50-lr1e-06-d0-1-tdf8d21-vkgdn_prefill_backend=triton-sp8e0184`
——协议三件套（6144/14336 + concise 提示 `-sp8e0184`）+ lr 1e-6 + **全量难度表**（`_full`，`-tdf8d21`）。
eval 端 `--proto_from` 回读同一训练 config，BASE 与四个 ckpt 严格同档，Δacc 公平。

#### 7.5.1 eval 结果（N=500 / greedy / seed=42 / dapo_math dev）

| 模型 | acc | fmt | 条件精度(acc/fmt) | Δacc vs BASE | McNemar | 判定 |
|---|---|---:|---:|---:|---|---|
| BASE | 57.8 | 67.6 | 85.5 | — | — | — |
| step50 | 60.0 | 70.0 | 85.7 | +2.2pp | p=0.338 (60/49) | 噪声内 |
| step100 | 61.0 | 69.2 | 88.2 | +3.2pp | p=0.178 (70/54) | 噪声内 |
| step150 | 61.4 | 70.0 | 87.7 | +3.6pp | p=0.136 (74/56) | 噪声内 |
| step200 | 62.2 | 70.0 | **88.9** | +4.4pp | **p=0.055** (71/49) | 噪声内（临界） |

（p 值已复核：step200 = 0.0548；表格里 0.055 是四舍五入。）

**读数（诚实版）**：

1. **方向干净但未确证**：四个存档点单调上升（+2.2/+3.2/+3.6/+4.4），分歧对 4/4 全正
   （b>c）。但无一达到 p<0.05，最近 step200 p=0.055 差一口气。N=500 下两臂差 ±6.2pp
   的固有地板（§9.1）决定了这是**功效问题不是信号强度问题**——按本次实测分歧对方向
   （59:41），80% 功效需 ~960 题，N=1000 同比例预计 p≈0.005。
2. **不成立 ≠ 反向成立**：按项目判据（McNemar p<0.05）写"方向一致、未确证"，与 §6.5
   提示层那轮的判据结局同类。不许写成"+4.4pp 增益"。
3. **跨 run 比较的 run 级噪声未进 p 值**（§7.3.1 定案 4 原样适用）：McNemar 只算题级
   采样噪声，没算 RL 轨迹分岔。且 p5 的 BASE=57.8 比 P1 时代（45.2）高 12.6pp——协议
   换新后基线整体抬升，p5 与老表**不可直接并排**。

#### 7.5.2 record 曲线（会话A：190 组 / gen_ver 0..176 / 09-17 18:26~23:21）

| 窗口（样本） | acc率 | fmt率 | 条件精度 | code率 | code_ok率 | trunc率 | avg_clen |
|---|---|---|---:|---:|---:|---:|---:|
| 0~160 | 61.3% | 74.4% | 82.4% | 65.0% | 55.0% | 21.9% | 4018 |
| 160~320 | 55.6% | 63.1% | 88.1% | 63.1% | 53.1% | 31.2% | 4741 |
| 320~480 | 66.9% | 80.6% | 82.9% | 88.1% | 71.9% | 12.5% | 4272 |
| 480~640 | 56.2% | 70.0% | 80.4% | 75.0% | 57.5% | 28.1% | 4808 |
| 640~800 | 66.2% | 76.9% | 86.2% | 73.8% | 56.9% | 23.1% | 4957 |
| 800~960 | 66.2% | 73.8% | 89.8% | 83.1% | 70.0% | 20.0% | 4544 |
| 960~1120 | 66.2% | 75.6% | 87.6% | 69.4% | 49.4% | 21.2% | 5559 |
| 1120~1280 | 55.6% | 72.5% | 76.7% | 63.7% | 57.5% | 28.1% | 4790 |
| 1280~1440 | 65.0% | 68.8% | 94.5% | 62.5% | 47.5% | 26.2% | 4914 |
| 1440~1600 | 62.5% | 70.0% | 89.3% | 60.0% | 38.8% | 27.5% | 5517 |

**与 eval 对账（同指标口径核对）**：

1. **eval 表里的 code% 是调用率（code_rate）不是成功执行率**——analysis.py 读 `code_rate`，
   eval_vllm_one.py 的 `code_rate` = 至少用过一次代码的题占比。所以 eval code% 与 record
   "code率"（code_used 口径）是**同一个指标**，可直接对账；eval 表不显示 code_ok_rate。
2. **同指标 level 差 ~2× 来自两个系统性原因，不是测量错误**：
   - **mode vs mixture**：eval 是 greedy 单条 argmax 轨迹，record 是 temp 1.0 的 8 轨迹混合。
     60% 的采样概率完全可以与"argmax 不写代码"共存——reward 侧 retool_math `code_w=0.0`
     （reward.py:289-291，"code 仅作监控记录，不进 reward"），模型用代码只有间接收益。
   - **分布**：训练池 = 难度过滤后的 1.16%（band 内中难题，更需工具），held-out dev = 原样。
3. **两条干净趋势（各自内部分辨率保证）方向一致**：
   - eval 四存档点同一 dev 集同一 greedy：调用率 63→66→52→**29**，是真实行为迁移
     （BASE 44.8 / step50 63.2 健康，机制在跑；若沙箱挂了 acc 会跟着崩，而 acc 在涨）。
   - record 每次调用成功率（code_ok/code_used）：55/65=84.6% → 49.4/69.4=71.2% →
     47.5/62.5=76.0% → 38.8/60.0=**64.7%**。代码分支在持续变弱——这正是 greedy 调用率
     腰斩的机制基础（代码分支后验让位给直接作答分支）。
4. **这是健康的重新分配，不是 run2 式崩塌**：关键在 acc 方向。run2 m600 崩塌是 code
   62.6→13 同时 acc 掉回 base 之下（46.4）；这里 code 越用越少、acc 越涨越高（62.2）、
   条件精度越涨越高（88.9）——"少依赖工具、直接做对"，正是 concise 提示设计方向
   （"As soon as you have the answer, stop immediately"），与 p4 的 code_ok 缓降但 acc
   高位震荡一致。
5. **唯一残余疑点**：代码分支成功率掉到 ~65%，两种读法——(a) 良性：只在最难题才用
   代码，难度选择拉低成功率；(b) 需警惕：代码书写能力退化。判据在 eval items 里就有：
   跨存档点按题看 `code_used=1` 的 `code_ok`。末窗 code_ok 38.8% 是全程最低，最终 ckpt
   的 code_ok 可能更低，顺手核一下。

#### 7.5.3 读数陷阱修正：record 的 clen_cap 是旧协议硬编码

`summarize_record` 的 `clen_cap=1800` 是 `max_context_tokens(2200)−prompt(400)` 时代的
常数（analysis.py:204）。p5 是 **ctx=14336 / prompt=1024 → 真上限 ≈13312**，于是
`0.9×1800=1620` 的"≥1620"标记（73~96%）纯属饱和噪声，无信息量。**正确读数**：avg_clen
4.0~5.5k 只占真预算 30~40%，全程稳定无上行漂移——**无长度失控**（早先担心的"clen 5399
抬头"不成立，960~1120 到 5559 又回落，是震荡不是趋势）。

修法：`summarize_record` 读 record.jsonl 同目录的 run_info.json 推导
`clen_cap = max_context_tokens − max_prompt_length`（缺文件回落 1800），加测试。

#### 7.5.4 判定与下一步（预写判据，跑之前先定死）

**当前判定**：step200 +4.4pp、McNemar p=0.055 → **方向一致、未确证**。与 §7.4 判据
（"p4s200 显著为正 + 条件精度 ≥85% 保持"）对照：条件精度 88.9 ≥85 ✓，显著 ✗。

#### 7.5.5 p5 加量终局 + 增益被抹平定案（2026-09-18）

**执行**：A（加量 `--all_steps 300 --save_steps 50` 新 out_dir）+ B（`--gen_update_steps 8`）
合并到一次 run。签名 `retool_math-ts0-ol1-r2x6144-s300x50-lr1e-06-d0-1-tdf8d21-...`，
全量难度表，2320 样本 ≈290 组（设计剂量 300 步/2400 轨迹的 96%），lr 1e-6（§7.4 定版）。

**eval（N=500 / greedy / dev，BASE 同档协议）**：

| 模型 | acc | fmt | code% | Δacc vs BASE | McNemar | 判定 |
|---|---:|---:|---:|---:|---|---|
| BASE | 59.6 | 69.6 | 45.6 | — | — | — |
| step100 | **64.2** | 74.4 | 26.6 | **+4.6pp** | **p=0.033** (b=65/c=42) | **显著** |
| step200 | 60.8 | 72.8 | 3.0 | +1.2pp | p=0.634 | 噪声内 |
| step300 | 56.2 | 66.8 | 3.0 | **−3.4pp** | p=0.145 | 噪声内 |

**记录曲线（会话A，290 组）**：code_ok 率 56.2%(0~160) → 42.5% → … → 28.1%(960~1120) →
6.9%(1120~1280) → **10.0%(2240~2400)**。code 分支成功率全程单调崩，与 eval code% 45.6→3.0
同构。

**三条定案**：

1. **计划 A 已执行且判据已给到：step100 显著（+4.6pp, p=0.033），但增益在 step300 被抹平
   （−3.4pp）**。这不是"没效果"——step100 是独立显著的正增益；这是"效果没保持住"。
   p5 加量版本 = run2（m200 +5.8pp → m600 +0.6pp）的**同款模式第三次出现**：
   早期存档点显著、晚期回吐，且 code 率随 acc 一起掉。
2. **「快照时机错位」已排除**：本轮 `--gen_update_steps 8`（run_info 确认），step_N 里
   最新权重产物占比从 ~1/16 提到 ~1/8，但 step300 仍回吐——错位不是主因。
3. **「测量被基准掩盖」未排除**：BASE 59.6 是 6144/concise 档，训练增量在 greedy dev eval
   上仍可能被压缩。**分布内对照（--split train）仍未做**（计划 C 待跑）。

**为什么"看不到效果"的深层判断（更新）**：剂量不足（§7.5.4-1）与快照错位（-2）两个
"看不到"因素本轮都已解决，方向一致且在 step100 显著——但**方向在 step300 反转**，
这不是"没看到"，是"看到了又消失"。机制高度指向 §7.5.2-5 预判的代码分支退化：
code_ok 率 56%→10% 单调崩（远低于预判的 ~65%），模型"理性"放弃工具、转纯推理。
**判别 H1（理性压灭，无解）vs H2（激励不足，可救）落在代码分层分析（`--code-layer`）**：
eval per-item 里 code_used=1 的 acc vs code_used=0 的 acc——用码样本 acc 更高 ⇒ H2（可救，
杠杆=尝试级 shaping 或加大 cold code 权重）；不高/更低 ⇒ H1（4B/GSM8K 上代码路径无真实
优势，崩塌是 RL 的最优解，诚实收尾，参照 §7.5 适用域结论）。

**下一步行动（一条做完再动下一条）**：

- **①（零成本，已落地工具）**：`python -m rlab.analysis --code-layer eval_vllm_all.json`
  分层分析，钉死 H1/H2。

#### 7.5.6 分层/迁移/双 split 定案 + 方案 B 落地（2026-09-18）

**代码分层分析（--code-layer，dev per-item）**：增益 100% 来自纯推理层（step100 +14.4pp
p=0.001 / step200 +13.2pp p=0.000 显著），用码层相对 BASE 无增益（-3.4pp 噪声内，n=90
配对够大）。**代码压灭是理性的（H1 方向）**——但方向与 §7.5.5 的 H2 预判相反。

**分层迁移分析（--code-migration，同题双 acc）**：以 BASE 分层为锚，500 题全量对账：

| 存档点 | 用码层 Δ（228题） | 纯推理层 Δ（272题） | 整体 |
|---|---:|---:|---:|
| step100 | -3.9pp | +11.6pp | +4.6pp |
| step200 | -10.9pp | +11.3pp | +1.2pp |
| step300 | **-12.7pp** | **+4.6pp** | **-3.4pp** |

BASE 用码题 acc 74.1% vs 纯推理题 47.4% = **代码有真实价值 +26.7pp**；训练把用码
228→8 题、放弃代码的题 acc 从 74.1% 掉到 62.3%。**净效果 -3.4pp = 丢代码（-12.7pp×228）
远超涨推理（+4.6pp×272）。**

**双 split 对照（--split train 修复后带难度过滤；dev 为 held-out）**：

| 模型 | train acc | Δtrain | dev acc | Δdev |
|---|---:|---:|---:|---:|
| BASE | 69.0 | — | 59.6 | — |
| step100 | 71.8 | +2.8 (p=0.272) | 64.2 | +4.6 (p=0.033) |
| step200 | 72.0 | +3.0 (p=0.248) | 60.8 | +1.2 (p=0.634) |
| step300 | 67.0 | **-2.0** (p=0.463) | 56.2 | **-3.4** (p=0.145) |

**五条定案**：

1. **代码压灭跨分布一致**：train/dev 两个独立抽样里 code% 都 ~50%→2.6-3.0%。不是评测
   集假象，是训练真的在杀代码。
2. **训练退化跨分布一致**：step300 在 train（-2.0）和 dev（-3.4）都负向。不是分布外
   泛化问题，是模型后期整体变差。
3. **step100/200 是真实的、小的增益**（train +2.8/+3.0、dev +4.6/+1.2，方向全正，幅度
   在噪声边缘）。**有效果但小，且被后期退化吃掉。**
4. **根因收敛到协议结构，不是参数**：lr 1e-6 / reward ±1 / clip 0.2-0.28 全对齐官方与
   参考；`max_rounds=2` 只有 1 次代码机会——代码写错一次整条轨迹就废、无补救空间 →
   RL 算出"写代码期望净收益为负"→ 理性压灭（与 §4.2 预判一致）。
5. **参考项目（agentic-rl-lab/05-retool，同 Qwen3.5-4B）证明多轮能保代码**：
   `max_code_calls=4/turns=6` 让代码可分步改进——code_calls 1.24→1.98 单调升、
   Average@12 23.6→47.5（+23.89pp，正好是用户要的理想状态）、sandbox success
   0.68→0.80（代码越练越能跑）。它预算：6 轮×1024、轨迹≤8192、LoRA r32 lr 4e-5、
   原生 `<tool_call>` 协议（base 工具调用率 87.5% 起点高）。

**方案 B 落地（commit 1e186c1，2026-09-18）**：preset `max_rounds 2→4`（3 次代码机会）、
`round_gen_tokens 3072→2048`（不赌 4B 在 1024 下的截断风险，docs/03 探针 1024 截断 85%）、
`max_context_tokens 8192→14336`（need=4×2048+1024+798=10014≤14336 ✅，p5 实测档）。
lr/reward/clip 全不动（单变量）。配套 eval 加 `--val_n` 采样评测（Average@N，
temp 1.0/top_p 0.7 参考口径）——greedy 会把多轮代码测成灭绝（docs §7.5.2 mode vs
mixture），参考的增益全在采样下显现。

**运行**（新 out_dir，撞名护栏强制）：
```bash
bash rlab/run_gsm8k.sh retool_math /root/Qwen3.5-4B \
  --all_steps 300 --save_steps 50 --gen_update_steps 8 \
  --difficulty_path rlab_out/difficulty_probe_4b_v5_r6144_concise_full.jsonl \
  --out_dir rlab_out/retool_math_p6
# 评测（采样档，BASE 经 --proto_from 同档）：
python -m rlab.eval --algo retool_math --n 200 --seed 42 --val_n 8 \
  --models "step100=./rlab_out/retool_math_p6/step_100,step200=./rlab_out/retool_math_p6/step_200,step300=./rlab_out/retool_math_p6/step_300" \
  --out eval_vllm_all_p6.json
```

**判据（跑之前定死）**：① code% 不再单调灭绝（≥30% 保持）→ 方案 B 止住压灭；
② code_calls（avg_rounds）>1 且不降 → 模型在学"多试几次"；③ Average@8 acc 单调升或
step200 显著 > BASE → 多轮让代码 advantage 转正。任一不满足 → 方案 B 证伪，转 A
（6×1024 完全对齐参考）或接受 §7.5 适用域结论。
- **②（一次评测）**：`--split train` 分布内对照——区分"分布内增益被基准掩盖" vs
  "分布外就是没增益"。
- **③**：若 H2 → 尝试级 shaping（code_used>0 给小分，不依赖 code_ok）或 cold code 权重
  单变量实验。**现在不动 lr、不动 outcome-only reward 本体**（改 reward = 动协议）。
- **不要**因为 step300 回吐就判"整轮无效果"——step100 显著是事实，回吐本身是要诊断的
  现象（代码分支退化），不是结论。



原计划的 `--models m200=...step_200_mm` 已不可执行。替代方案分两层：

1. **p1s200 vs 已灭失的 m200**（同 step、唯一差异 trunc_shaping —— 最干净的单变量对）：
   走 per-item json 遗物。前提：**先备份 m200 的评测 json**（它是唯一遗物）：
   `cp eval_vllm_all.json eval_vllm_all.run3-0913.json.bak`（含全部 items）。
   评测 p1s200 后：
   ```bash
   python -m rlab.analysis --eval-json eval_vllm_all.json \
     --pair-json eval_vllm_all.run3-0913.json.bak --pair-a p1s200 --pair-b m200
   ```
   同 seed/split/n 抽同一批题，qk 配对成立；两边都是 09-13 新代码产物，items 齐全。
2. **P1 曲线**：p1s150/p1s200/p1s250/p1s300 vs BASE/baseB，看 50 步粒度上增益何时出现、
   末段是否延续训练曲线的上爬。

解读时记住 §7.2 的限制：即使 p1s200 ≈ m200（差在噪声内），也只能说明"shaping 在
step200 处不是增益来源"，不能说明"shaping 不是晚期崩塌的原因"（P1 没跑到那段）。

### 7.3.1 P1 eval 结果与定案（2026-09-13，N=500 / greedy / seed=42）

| 模型 | acc | fmt | code | Δacc vs BASE | McNemar | 判定 |
|---|---:|---:|---:|---:|---|---|
| BASE | 45.2 | 51.2 | 42.2 | — | — | — |
| baseB（同路径重跑） | 45.4 | 51.2 | 42.2 | +0.2pp | p=1.000 (b=2/c=1) | 噪声内（仅 3 题翻转） |
| p1s150 | 37.8 | 41.6 | 56.2 | **−7.4pp** | **p=0.001 (b=43/c=80)** | **显著更差** |
| p1s200 | 45.2 | 51.8 | 57.6 | +0.0pp | p=1.000 (b=61/c=61) | 噪声内（与 BASE 精确打平） |
| p1s250 | 43.0 | 53.8 | 59.8 | −2.2pp | p=0.371 | 噪声内 |
| p1s300 | 43.8 | 54.0 | 58.8 | −1.4pp | p=0.516 | 噪声内 |

**跨 json 配对（§7.3 通道）**：m200 − p1s200 = **+6.4pp，p=0.002 (b=37/c=69)** —— m200
显著更好。5 重 Bonferroni（0.05/5=0.01）下 p1s150 的 p=0.001 仍显著。

定案四条：

1. **§4.1 证伪（方向相反）**：同 step200、唯一差异 trunc_shaping —— 留着 0.5 的 m200
   51.6，去掉的 p1s200 45.2。惩罚是增益的组成部分，"装反/灭工具"叙事终结。
2. **中段崩盘坐实到模型本体**：p1s150 显著低于 BASE —— 无惩罚时"自由变长撞上限"
   的崩盘不是取样假象（训练侧组 100–224 与 eval 端一致）。
3. **P1 末段无可 eval 增益 → 不延长**：p1s250/300 都在 BASE 之下（噪声内）；
   训练曲线末段"还在爬"没有翻译成 greedy 增益。
4. **一个诚实的口径警告**：m200−p1s200 是**跨 run** 比较 —— McNemar 只算了题级
   采样噪声，没算 run 级随机性（RL 轨迹分岔）。方向有训练窗口佐证（组 300–400：
   48.2 vs 58.4），但量级要靠 P1b（同配方重跑 run2 的 ts0.5、300 步、存盘 50）定标。

**下一步 = P1b（优先于 P2）**：用 run2 原配方（ts0.5）重跑 300 步 / 存盘 50 /
**新 out_dir**（撞名护栏会强制）。一次花 2.6h 回答两件事：
① m200 的 +5.8pp 是配方属性还是 run 随机性（新 s200 vs BASE vs 遗物 m200 三方配对）；
② 拿回一个**活着的**最佳 ckpt —— m200 已灭失，OOD（AIME25）和后续 P2 对照都需要它。

---

## 8. 反建议（明确不要做）

- ❌ **不要**继续在围栏协议上抬高单轮预算 —— 治标，且继续挤压轮数。
- ❌ **不要**把 final / step1000 当主结果 —— m600 增益已消失（+0.6pp vs BASE，p=0.796），
   好东西在 step≈200，晚段存盘只在把曲线锯齿看清楚时有价值。
- ❌ **不要**引用旧版 `analysis.py` 的"真差异"标签 —— 它是 GSM8K/N=300 口径的常数。
- ❌ **不要**延长 P1（ts0）或给它加步数 —— p1s250/300 均在 BASE 之下（噪声内），
   训练末段的上爬没有翻译成 greedy 增益（§7.3.1 定案 3）。
- ❌ **不要**在补上 OOD 集之前宣称复现了 ReTool —— 参考的 +23.89pp 是 OOD。
- ❌ **不要**一次改多个变量 —— 尤其别把 P1（奖励）和 P2（协议）合到一次跑里。
- ❌ **不要**用 run2 的 `eval_vllm_all.json` 做配对检验 —— 旧产物永远只有聚合值，补不回来。
- ❌ **不要**以为 run2 的 `step_200` 原始 ckpt 还在 —— **已被 P1 覆盖**（§7.2.1）；
   其后 `step_200_mm` 也被删除，**m200 模型已灭失**，与它的一切新对照只能走
   per-item json 的 `pair_eval`（§7.3），别再拿模型路径说事。

---

## 9. 附录 A：测量口径修正（本次已落地）

### 9.1 为什么必须改

旧版 `summarize_eval` 用**固定 ±2pp 地板**判"真差异"（`NOISE_FLOOR_PP`），那是 GSM8K/N=300 时代的常数。
实测量级：dapo_math **N=500** 下**单臂 95%CI 就有 ±4.4pp、两臂差 ±6.2pp** —— 于是 m200−BASE 的
+5.0pp（未配对 p≈0.11）会被直接打成"真差异"。同时旧版 eval 只存聚合计数，**同题配对检验永远算不出来**。

### 9.2 改了什么

| 文件 | 改动 |
|---|---|
| `eval_vllm_one.py` | 新增 `--dump_items`（`BooleanOptionalAction`，**默认开**）；每题记 `qk`（题面 sha1 指纹）/`acc`/`fmt`/`code_used`/`code_ok`/`ans_len`/`empty`；结果加 `model_path` + `eval_protocol`；顺带修空答案分支的 `a`/`f` 未赋值隐患 |
| `eval_vllm.py` | 聚合 json 加 `_meta`（`base_path`/`tuned`/抽样口径/`failures`/`created`）；`analysis.py` 汇总时跳过 `_` 前缀键 |
| `rlab/analysis.py` | 新增纯函数 `ci95` / `diff_ci95` / `mcnemar_exact` / `paired_counts` / `_verdict`；`summarize_eval` 输出 CI 列 + 检验列；**有 per-item 用 McNemar，没有则明确标注"（无 per-item）"**，不冒充满配检验 |
| `rlab/train.py` | `run_signature(cfg)` + 三处冗余；`run_info.json` 顶层补 `save_steps`/`max_rounds`/`trunc_shaping`/`overlong_shaping`/`difficulty_path`/`difficulty_band`/`lr`（完整 `config` 仍保留） |
| `rlab/tests/test_smoke_cpu.py` | 新增 `[G]` 段 19 项（原 45 → **64 项**） |
| `rlab/tests/test_retool_cpu.py` | `run_info` 验收补 3 项签名接线检查 |

### 9.3 run 偏离签名

```
retool_math-ts0.5-ol1-r2x3072-s1000x200-lr5e-06-d0-1     ← run2 实跑
retool_math-ts0-ol1-r2x3072-s300x50-lr5e-06-d0-1         ← P1 拟跑
retool_math-ts0-ol1-r6x1024-s300x50-lr5e-06-d0-1         ← P2 目标
retool_math-ts0.5-ol1-r2x3072-s300x50-lr5e-06-nodiff     ← P4 对照
         └algo  └trunc └ol └轮次×单轮 └步数×存盘 └lr    └难度带
```

三处冗余：启动日志 `偏离签名 signature=`（grep 即得）、`run_info.json` 顶层、wandb run name。

### 9.4 用 run2 真实数字验证

> 下表是**旧代码产物、无 per-item** 的演示（当时 BASE=46.6 的一次运行），只用于展示口径变化。
> 定案数字以 §7.1 的 2026-09-13 配对重评为准（BASE=45.8，m200 判定相反：显著）。

```
> N=500 · 单臂 95%CI 最坏 ≈±4.4pp · 两臂差 ≈±6.2pp · 判定 = CI 不跨 0（有 per-item 时改用 McNemar p<0.05）

| 模型 | acc%(±95%CI) | fmt% | code% | Δacc vs BASE | 检验 | 判定 |
| BASE | 46.6±4.4 | 53.0 | 42.2 | — | — | — |
| m200 | 51.6±4.4 | 60.2 | 62.6 | +5.0±6.2pp | 两比例（无 per-item） | 噪声内 |
| m600 | 46.4±4.4 | 67.8 | 13.0 | -0.2±6.2pp | 两比例（无 per-item） | 噪声内 |
```

### 9.5 使用注意

- **pod 上先 `git pull` 再评**，并自检新代码在位：
  `grep -c dump_items eval_vllm_one.py`（>0）。判据还可以看 json：三个模型都应有
  `items`/`model_path`/`eval_protocol`，顶层应有 `_meta` —— 缺任何一项就说明仍在跑旧代码。
- 命令**不变**：`--dump_items` 默认开，wrapper 不转发也能生效。flag 拼写是**下划线**
  （`--no-dump_items`；argparse 不接受 `--no-dump-items`）。它**只能直接给 `eval_vllm_one.py`**，
  `eval.py`/`eval_vllm.py` 暂无透传出口。
- **配对不要求一次调用评完所有模型**：`random.seed(seed)` → `random.sample(test_data, n)`，
  `test_data` 顺序确定 ⇒ 同 seed/split/n 的不同次调用抽到**同一批题**，跨次也能靠 `qk` 配对；
  若某次改了 `--n`，则只按交集配对，表格 `n=matched` 会显示实际配对数。
- **旧的 `eval_vllm_all.json` 重跑即被覆盖**，先 `cp` 备份（它是 run2 唯一的聚合记录）。

### 9.6 run-to-run 噪声实测（baseB 对照，2026-09-13）

同一权重（`/root/Qwen3.5-4B`）、同一命令、同 seed/split/n 连续两次评测：**仅 9/500 题结果翻转**
（b=2/c=7），净 −1.0pp，McNemar p=0.180。结论：

- vLLM greedy 并非逐位确定；** BASE 类模型的 run 噪声地板 ≈ ±1pp / ~2% 题目**。
  推测机制：近并列 logits 上批内执行顺序/归约顺序差异导致采样翻转（m200/m600 无此现象，
  说明策略输出更远离平局区）。
- 任何 |Δ| < ~1pp 的单模型变化（包括"两次评测 BASE 不一样"）**默认按 run 噪声处理**，
  除非配对检验给出 p<0.05 —— 这正是 baseB 对照存在的意义，重要评测建议保留该臂。

---

## 10. 附录 B：数据侧待核对

`filter_qas_by_difficulty`（`rlab/data.py:312-313`）把"不在探针表里"的题**静默丢弃**（`missing` 口径，
注释："过滤即选择，半覆盖的表不该让未探测题混进训练分布"）。run2 日志：

```
[rollout] 难度过滤 band=(0.0,1.0): 1791200 -> 20792 题（全错 23196 / 全对 8695 / 区间外 0 / 表中缺失 1738517）
```

两点需要查证：

1. **97.1% 的池子是因"未探测"被丢，不是因难度被丢**（23196+8695 = 31891 ≠ 20792，也不等于 17912）。
   即训练分布 = 探针表恰好覆盖到的那部分，**探针覆盖率是承重结构**。
2. **池子总量 1,791,200 与 DAPO-Math-17k 的 17.9k 相差约 100×** ——
   需确认 pod 上 `rlab/datasets/dapo_math/train.jsonl` 到底是 1.79M 行还是 17.9k 行（是否重复展开）。

与 §4.4 的退化率差异（参考 ≈0 / run2 14–16%）对照，这两点可能相关。**优先用一次无
`--difficulty_path` 的 300 步对照跑回答"过滤是否真在降退化"。**

---

## 11. 变更文件与验证状态

| 文件 | 状态 |
|---|---|
| `eval_vllm_one.py` | 已改（per-item + 审计字段，09-12） |
| `eval_vllm.py` | 已改（`_meta`，09-12） |
| `rlab/analysis.py` | 已改（统计口径 09-12；**`pair_eval` 跨 json 配对** 09-13，对照已灭失模型） |
| `rlab/train.py` | 已改（偏离签名 09-12；**ckpt 撞名护栏** 09-13：`guard_ckpt_collision` 启动扫描 + 存盘二次核对） |
| `rlab/health.py` | 已改（09-13：`maybe_check` 滚动门，修 P1 检查点盲窗） |
| `rlab/run_gsm8k.sh` | 已改（09-13：旧 `step_*` 默认拒绝启动 + `OUT_DIR` 透传给 train） |
| `rlab/tests/test_smoke_cpu.py` | 已改（+19 项 09-12、pair_eval +3 项 09-13；`[H]` 抽取自检 +4 项 09-14，**71 项全过**） |
| `rlab/tests/test_retool_cpu.py` | 已改（run_info +3 项 09-12；健康滚动门 +2、ckpt 护栏 +4，09-13；**09-14 修脆性断言**：`[T]` 段 `--attn_implementation "$ATTN_IMPL" "$@"` 的字面紧邻判据被 8496f47 插入的 `--out_dir "$OUT_DIR"` 打断，改断言**位置关系**；09-14 vLLM 引擎参数透传 +12 项，**301 项全过**） |
| `rlab/extract_text_model.py` | 已改（09-14：`_selfcheck` 三层判据 + 反证控制，见看板 09-14 行） |
| `rlab/readme.md` | 已改（测试计数与 eval/analysis/train/health 说明） |

验证（2026-09-14 本机实测）：
- `python -m rlab.tests.test_smoke_cpu` → **71 项全过**
- `python -m rlab.tests.test_retool_cpu` → **301 项全过**（`[T]` 断言修复前 exit=1：
  该段是文件靠后的段落，中断导致其后 4 个段从未执行 —— 见下方教训）
- `test_health_monitor()` / `test_grad_clip_and_run_info()` 隔离运行 → 全过
- `bash -n run_gsm8k.sh` 通过；全部改动文件 `py_compile` 通过

> **教训（2026-09-14）：「隔离跑几个子测试」≠「跑整个文件」。**
> 本表此前记的"test_retool_cpu 共 66 项、隔离运行全过"——隔离只覆盖了 health / grad_clip
> 两段，而失败发生在**文件最后**的 `[T]` 段，且因断言在 [T] 处抛异常，其后的 4 个段
> **根本没执行**（这也解释了计数长期对不上：实际 280 项，文档记 66/67）。
> 该断言自 8496f47（09-13）起就是红的，但**不影响 pod 行为**（脚本语义完好，坏的是
> "字面紧邻"这个过强的实现细节），所以只有本地跑完整文件才会暴露。
> 另：早前记的"开发机 `test_retool_cpu` 跑到 transformers 段 `ModuleNotFoundError` 退出"
> 现已不成立（本机环境已补齐），红灯是这次才浮出来的。
