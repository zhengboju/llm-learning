...
3. **主嫌疑 `trunc_shaping=0.5`（P1 消融后削弱）**：我们与参考唯一的奖励差异，且它是
   **唯一与轮数正相关的惩罚**，参考专门讨论过 shaping 并选择不加（§4.1）。
   但 P1（ts0，300 步，2026-09-13）训练侧显示：去掉它后 acc/code 与 run2 同位置持平、
   截断率 **+14pp**、中段反崩一次（§7.2）—— 它更像在起**正常的压制截断作用**，而非
   崩塌主因。最终判定等 P1 eval；主嫌疑向第 4 条（协议/轮次）转移。
