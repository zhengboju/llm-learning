...
**为什么"看不到效果"：三件事叠加，不是没效果**（2026-09-17 复核 train.py 后确认）：

1. **剂量不足（单位修正后仍成立）**：`all_steps=200` 的 step = 1 组 = num_pre_Q=8 样本
   = 8 轨迹（train.py 的 step 就是组，GAS=4 步才一次优化器更新）。p5 实测 190 组
   =1520 轨迹 = 设计剂量（300 步/2400 轨迹）的 **63%**；p4 归档时 110 组 =880 轨迹 =
   设计剂量的 37%。即 p5 已超过 p4 的归档量（1.7×），但仍没到 §7.4 判据设计的 300 步。
   且每 `gen_update_steps=16` 步只有一批是新鲜权重产物（窗口内 staleness 0→16、
   平均≈8 micro-step），其余在复用旧权重数据。
2. **快照时机错位**：eval 评的 step_N 是 16 步推送周期里的"中间态"——每次推送后直到
   下次推送前的 15/16 步都在吃旧权重产物，单个存档点里最新权重产物只占
   ≈1/16×1600≈100 轨迹。评"最新鲜权重"应评 gen_version 对齐的存档点，或调小
   `gen_update_steps`（如 8）。
3. **测量被相对基准掩盖**：BASE 57.8% 是 6144/concise 档（探针 correct 56~61%），协议
   本身就把模型推到接近上限，训练增量在 greedy dev eval 上被压缩。需要 `--split train`
   的分布内对照。

**下一步行动（按投入产出排序，一条做完再动下一条）**：

- **A（最优先）**：p5 加量到判据设计的剂量——`--all_steps 300 --save_steps 50` 重跑
  （同 out_dir 会被撞名护栏拦，换新 out_dir），eval step200/step300 vs BASE。
- **B**：eval 对齐新鲜权重——评 gen_version 对齐存档点，或 `--gen_update_steps 8`。
- **C**：eval 加 `--split train` 分布内对照（区分"分布内 vs 分布外"增益）。
- **D**：若 A 后仍平台，再考虑 lr 微调。**现在不动 lr、不动 reward**（retool_math 是
  outcome-only，`code_w=0.0`；改 reward = 动协议）。
- **不要**因为"看不到效果"在 N=500 时判"无效果"（功率只够看 >6pp，见 §9.1）。
