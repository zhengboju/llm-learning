...
> - `rlab/tests/test_native_protocol.py`：**172 项** CPU 测试（P1–P7 + 打分域 + P2b 表示层容忍全覆盖）。
...
| **护栏误杀**（校验把表示层差异当不同源） | **高·已实际发生两次** | §10.6.2：校验① 判据最终定为**解码文本相同**（覆盖 strip 与 BPE 段边界合并两种表示差异），P2b 组双向锁死 |
...
| `rlab/tests/test_native_protocol.py` | **新文件 ~1100 行** | 172 项 CPU 测试（含 P2b 表示层容忍组 + 报错窗口字符对齐锁） |
...
`test_native_protocol` 单跑 → **172 项全部通过**（P2b 表示层容忍组为 2026-09-25 两次真机事故后新增）。
...
# ③ 20 步验证跑（参考的"两杯瑞幸"档）
#    【2026-09-25 第二次上机】连撞两次护栏误杀——同一位置（样本 25 第 5 段）
#    报同一形态（1556 vs 1557 @1484），但根因不同：先是模板 strip，再是 BPE
...
这一类事故在真机上**连续发生了两次**，报错形态几乎一样，但根因不同层级。两条都记，
因为第二次比第一次更根本——它证明的不只是"判据写窄了"，而是"**token 序列全等这个
判据本身不可满足**"。

---

#### 事故 A（第一次）：模板 strip 了 assistant 内容
...
**修复 A**：校验① 加"只差空白 → 容忍"（`_drop_ws_tokens`）。

---

#### 事故 B（第二次）：**BPE 分段编码 ≠ 整串编码**（判据本身错了）

修完 A 再跑，**同一个样本 25、同一个第 5 段、同样是 1556 vs 1557 @1484** 又炸了，
但这次形态不同：

```
模板侧 [1484:1492] = [91450, 17, 7461, 198, 1302, 925, 91748, 64208]   ← 1 个 token
拼接侧 [1484:1492] = [2387, 332, 17, 7461, 198, 1302, 925, 91748]      ← 2 个 token
```

**关键读法（一眼可辨）**：`17, 7461, 198, 1302, 925, 91748` 后面**逐位重合**，
差别只是"1 个 token vs 2 个 token"，总长恰好差 1，两侧解码出的**文本完全相同**。

**这不是"不同源"，这是 BPE 的固有性质**：分词不满足"分段可加"。
本机 Qwen2.5 实测（`bpe_boundary.py`）：

```
encode("```") + encode("\n")  = [73594, 198]
encode("```\n")               = [13874, 3989]      ← 同一文本，token 不同
encode("a") + encode("\"") + encode(")\n") ≠ encode("a\")\n")
```

**为什么会命中**：`prev` 是**逐段累积**出来的（每段 observation 由模板渲染后切增量、
再拼到上一段之后），而校验里的 `canonical_prompt` 是模板**整串一次成串**重渲染的。
两条路径在**段边界**上必然可能出现这种合并差异——**这与内容无关，只与"边界字节恰好
能合并"有关**，所以它是**随机、迟早**发生的（这也解释了为什么同一个样本、同一个位置
连炸两次：那一段的边界字节就是能合并的）。

**事故 B 的根本教训**：旧判据（无论最早的逐 token 全等，还是事故 A 修的"只容忍空白"）
都在**token 序列层面**要求相等——而这个要求**本质不可满足**，因为：

1. 我们**主动选择**了 token-in token-out（docs/05/09 明写"text↔token 重编码不可逆"，
   正是为此放弃整串重渲染）。既然不复用模板的重分词，两条路径的分词结果**就不可能
   保证逐 token 相同**；
2. 真正要守的不变式是**"两条路径表示同一段历史文本"**（内容层面），而不是"token 序列
   相同"（表示层面）。而且**生成端与训练端用的是同一条 `prev` 序列**（由构造保证，
   见 `retool_build_batch`）——**gen_logps 与序列永远对齐**，这才是契约的实质。

**修复 B（最终判据）**：校验① 的容忍口径改为**解码文本（去空白）相同**：

```python
_cv = _norm_text(tokenizer, canonical_prompt)   # token ids → 去空白文本
_pv = _norm_text(tokenizer, prev)
if _cv is not None and _cv == _pv:   # 文本同源 → 容忍（strip / BPE 边界都覆盖）
    ...（只告警一次，附 token 数差）
else:                                 # 文本也不同 → 真不同源 → raise
    ...（报错带双方分歧位的**解码文本**）
```

`_norm_text` 一次 `tokenizer.decode(整段)` + 正则去空白（1400 token 实测 **6.7ms/次**，
且只在 token 序列**不等**时才走到——不等本身是少数事件，热路径仍是零开销的
`canonical_prompt != prev` 快路径）。

**为什么"去空白"而不是"逐字符全等"**：模板对 assistant 内容 strip 是**既成事实**
（事故 A），且空白不承载语义；去掉它使判据只对"可见内容"敏感——这正是我们要守的层。
**任何可见内容差异**（tools 档不一致、`enable_thinking` 档不一致、模板改写历史、
段中 A vs B）都会让文本不同 → **仍然 raise**，校验① 的本职一点没丢。

**残留风险（如实声明）**：判据停在"解码文本"，故"文本相同、token 不同"的差异**不会
被发现**。这是**刻意**的，不是漏洞——这类差异本就是两条合法分词路径的产物（见上），
且生成/训练/打分三方共用同一条 `prev` 序列，不存在"错位"可言。

**测试（P2b 组，19 项）**——两个方向都锁死：
- 正例：段首 `\n` / `\n\n` / 空格 / 首尾空白 → 容忍；**BPE 段边界合并（文本相同、
  token 数不同）→ 容忍**（新增，用 `BPETok` 子类显式模拟"整串合并 / 分段不合并"，
  复现真机 1556 vs 1557 形态）；且**返回序列仍以原样 prev 开头**（容忍 ≠ trim）。
...
- 端到端：5 段**每段都以换行开头**（最坏情况）→ 全过且逐 token 不改动采样 token
  （修复前第 2 段就炸）。
- **报错窗口假象**（元教训④ 的锁）：按 token 宽度取窗口会显出"内容不同"、按字符
  对齐则一致 → 4 项，确保**诊断工具本身不伪造差异**；另有占位符静态锁 2 项
  （`"x"` 是载荷常量，改成空白会让校验② 误杀）。

**突变测试（双向）**：删掉容忍 → P2b 失败 ✅；把容忍写成"无限容忍"（不看可见
内容）→ P2b + P3 双双失败 ✅；**把判据退回"只容忍纯空白 token"（即事故 A 的修复）
→ 新增的 BPE 用例失败 ✅**（`mutate_guard.py` 实测，证明新用例真的在测 BPE 容忍、
不是假测试）。三个方向都有测试兜住。

**元教训（已入全局）**：
> ① **护栏的判据必须停在语义层，而不是表示层。** 逐字节/逐 token 全等是最容易写、
> 也最容易把正确运行打死的判据——凡"两边各自渲染同一份历史"的比对，先问
> "这两条路径会不会对同一内容做等价的表示变换（strip/大小写/Unicode/BPE 分词）"。
> ② **当一种"表示差异"被容忍后，要接着问：还有没有同类差异没被覆盖？** 事故 A 修完
> 只覆盖了 strip，BPE 边界立刻以**同一形态**（1556 vs 1557 @1484）再次炸出来——
> 因为两次都只是"表示层"的不同侧面。**修判据要修到语义层，否则同类事故会排队发生。**
> ③ **报错信息要带解码后的文本，不只给 token id / 位置**——
> `1556 vs 1557 @1484` 三个数字花了很久才定位，而两侧文本一对照，
> "文本完全相同"是**一眼可见**的。
> ④ **但"带文本"还不够——跨分词比较时，文本窗口必须按字符对齐。** 这是这次
> 浪费最多时间的一处：修① 之后报错里改成各自解码 token 窗口，真机打出来的是
> 一侧 `…has no attribute 'now`、另一侧 `…has no attribute '`，**看起来"可见内容
> 不同"**，于是判断方向被带偏。实际两段文本**完全相同**——模板侧 1 个 token、
> 拼接侧 2 个 token（1556 vs 1557），**同样 64 个 token 的窗口覆盖的字符数不同**，
> 尾部凭空差出一截。本机已复现该假象并改为 `_char_aligned_span`（整段解码 →
> 字符域定位 → 取同一字符区间），P2b 有断言锁死"按 token 宽度取窗口会显出内容
> 不同、按字符对齐则一致"。
> **诊断工具自己也是被测对象**：一个会系统性伪造"差异"的报错，比没有报错更贵——
> 它会把人引向错误的方向。

**为什么这个 bug 没被 CPU 测试抓住**：MockTok 是**字符级**编码，其 docstring 明确
写着"BPE 的跨段合并**刻意不模拟**——那是真 tokenizer 的属性"。于是：
① 事故 A 的 strip 差异没被"原样采样 token vs strip 后重渲染"这个**真机必然出现**的
组合测到（已补，用 `strip_assistant=False` 造生成端序列）；
② 事故 B 的 BPE 差异**在字符级编码下不可能出现**，只能靠 `BPETok` 子类显式模拟
（已补）。
