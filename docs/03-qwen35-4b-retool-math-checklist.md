...
> 结论**已被 run2 数据与参考项目证伪**，见 05 文档 §4.2。
...
- [x] `round_gen_tokens`：**v4 探针实锤（2026-09-11）必须 3072**。机制：4B 关
  ...
  > 的轨迹成功跑过代码），提示层把它压到 0.441（"代码块短、无注释"的直接后果，
  > 不是弃用）。**结论：TIR 路径在 4B 上是活的**，"不必要"的判断不成立；
  > 待验证的是它是否**有效**（per-question corr(avg_code_ok, pass_rate) 受难度混淆，
  > 只能在训练期用同题组内对比来判）。
