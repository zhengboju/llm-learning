...
### 11. 验收门结果：**通过**（2026-09-16 11:0x，`retool_math` 4B 训练实跑）

命令（§10 的验收跑，`--verify_gen_logps 5` 只为这一跑）：

```bash
bash rlab/run_gsm8k.sh retool_math /root/Qwen3.5-4B \
  --chat_template_kwargs '{"enable_thinking": false}' \
  --vllm_gen_logps --vllm_batch_invariant --vllm_attention_backend FLASH_ATTN \
  --verify_gen_logps 5
```

**step 1 的 `[train][口径]`**（`staleness=0`、`mean_ratio=1.0001` ⇒ 两侧策略同一份，无训练动力学）：

| gen_logps 来源 | clip_frac | approx_kl | frac\|d\|>0.1 |
|---|---|---|---|
| vLLM 报的数（**未开**确定性档，§7） | 0.0087 | 2.23e-3 | 0.041 |
| torch 副本（doc 基线） | 0.0007 | 5.1e-4 | — |
| **vLLM 报的数 + batch-invariant** | **0.0008** | **5.73e-4** | 0.017 |

**0.0008 vs 地板 0.0007 = 1.14×**（未开档时是 12×）。近似 kl 5.73e-4 vs 5.1e-4 = 1.12×。**预设判据（"落到 0~1e-3、逼近 7e-4"）满足。**

训练端 verify 5 组（对照 §7 的未开档同表）：

| 组 | mean | p50 | p99 | max | >0.1 | >1nat | 前后半段 mean\|d\| |
|---|---|---|---|---|---|---|---|
| 1 | 7.39e-3 | 6.46e-6 | 0.115 | **0.588** | 1.75% | 0.00% | 6.49e-3 → 7.10e-3 |
| 2 | 5.65e-3 | 2.71e-6 | 0.110 | **0.784** | 1.40% | 0.00% | 5.98e-3 → 5.27e-3 |
| 3 | 5.65e-3 | 4.16e-6 | 0.106 | **0.258** | 1.26% | 0.00% | 6.01e-3 → 5.03e-3 |
| 4 | 1.11e-2 | 8.88e-5 | 0.129 | **0.720** | 2.77% | 0.00% | 8.77e-3 → 1.22e-2 |
| 5 | 1.21e-2 | 2.14e-4 | 0.129 | **0.454** | 2.91% | 0.00% | 1.08e-2 → 1.26e-2 |

与 §7（未开档）逐项对比：

- **`>1 nat` 五组全 0.00%**（未开档有一组 5.10、一组 3.23，且 0.03%/0.01% 的比例出现 >1nat 点）；
- **max 全部 < 0.8**（未开档 0.90 / 1.00 / 1.05 / 3.23 / 5.10）；
- **p99 收拢到 0.105~0.129**（未开档 0.148~0.252）——组间离散度也小了；
- mean 5.6e-3~1.2e-2（未开档 9.5e-3~2.0e-2），`>0.1` 占比约减半。

**一处需要纠正我自己的推理**（§9C）：我当时按 36 个诊断点说"没有 token 跨过 clip 阈值（±0.22/0.25）"。实际训练每组的有效位是 **5k~32k**，尾部确实会跨——这次的极值就是 `+0.59`、`+0.78`、`+0.72`（都是 vLLM > torch 的正向侧），`clip_frac=0.0008` 正是这条罕见的尾巴。所以准确的表述是：**尾巴高度被压掉了约 6 倍（5.10 → 0.78），不是尾巴消失**。36 点样本量太小，给不出"无跨越"这种强断言——数字预测（0~1e-3）蒙对了，机制解释当时说满了。

**吞吐（仍待定）**：本次 step 墙钟 338s（step 1，含引擎初始化 ~55s + torch 副本加载 + 5 组对拍）→ 36s → 13s，**这不是稳态值**，不能当结论；要看 20+ 步后的平均 s/step，并与 torch 副本档的同名数字比。按 §10 的账，这是切换与否的**唯一**未决项（显存 70G/96G 不紧张）。
...
5. **batch-invariant 档改写了这个结论的适用域**（§9）：可复现性与跨引擎口径都过关，**验收门也已通过**（§11：step-1 clip_frac 0.0008 ≈ 地板 0.0007）。代价只剩**吞吐未测**（§10）——这是切换与否的唯一未决项，**在此之前第 1–4 条就是当前纪律**。
6. **旧的"Qwen3.5 + vllm_gen_logps"警告已分档**（`rlab/rollout.py`）：开确定性档走提示分支（指向 §9 的实测），未开档才喊不可复现并给出两条修法。同一份代码库不能把"已修复"当"已知坏"用。
...
- 2026-09-16 11:0x 验收门**通过**：step-1 `clip_frac=0.0008`（地板 0.0007、未开档 0.0087）、`approx_kl=5.73e-4`、verify 五组 `>1nat` 全 0（§11）。剩吞吐一项待测。
